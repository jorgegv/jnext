...
int main()
{
    ...
    // --- SREP-01: a healthy Next at 50 Hz scores exactly 100% --------------
    ...
    // here and teach every user that jnext runs slow.
    {
        ...
        check("SREP-01a", "Next 50 Hz on target reports achieved 100%",
              r.achieved_pct == 100, got(r));
        check("SREP-01b", "Next 50 Hz on target is not a shortfall",
              !r.shortfall && r.achieved_valid, got(r));
        check("SREP-01c", "requested is the multiplier, unchanged",
              r.requested_pct == 100, got(r));
    }

    // --- SREP-02: same for the 60 Hz mode ----------------------------------
    ...
    // machine keeping up scores 100 in both video modes.
    {
        ...
        check("SREP-02", "Next 60 Hz on target also reports 100%",
              r.achieved_pct == 100 && !r.shortfall, got(r));
    }

    // --- SREP-03: integer-frame quantisation must not cry wolf -------------
    ...
    // are a machine that is keeping up; neither may be flagged.
    {
        ...
        check("SREP-03a", "49 frames in a 1s window is not a shortfall",
              !lo.shortfall, got(lo));
        check("SREP-03b", "50 frames in a 1s window is not a shortfall",
              !hi.shortfall, got(hi));
        check("SREP-03c", "quantisation stays within a point of 100%",
              lo.achieved_pct == 99 && hi.achieved_pct == 101,
              got(lo) + " / " + got(hi));
    }

    // --- SREP-04: an audio-pacer intervention must not cry wolf either -----
    ...
    // on top of quantisation: still not a fault.
    {
        ...
        check("SREP-04", "a one-frame audio-pacer skip is not a shortfall",
              !r.shortfall, got(r));
    }

    // --- SREP-05: a genuine shortfall IS reported --------------------------
    ...
    // for normal speed. The old indicator said "100%" here.
    {
        ...
        check("SREP-05a", "30 fps at 1x is flagged as a shortfall",
              r.shortfall && r.achieved_valid, got(r));
        check("SREP-05b", "30 fps at 1x reports ~61% achieved",
              r.achieved_pct == 61, got(r));
        check("SREP-05c", "the requested figure is still reported alongside",
              r.requested_pct == 100, got(r));
    }

    // --- SREP-06: the margin boundary, both sides --------------------------
    ...
    // becoming <= or the margin drifting.
    {
        ...
        check("SREP-06a", "exactly at the margin is not yet a shortfall",
              e.achieved_pct == 100 - SHORTFALL_MARGIN_PCT && !e.shortfall, got(e));
        check("SREP-06b", "one point past the margin is a shortfall",
              p.achieved_pct == 100 - SHORTFALL_MARGIN_PCT - 1 && p.shortfall, got(p));
    }

    // --- SREP-07: the margin is the DERIVED value, bounded on both sides ---
    ...
    // that still clears the noise, not merely some number above it.
    {
        ...
        check("SREP-07a", "margin clears 3 frames of window error at the worst period",
              SHORTFALL_MARGIN_PCT > budget,
              "margin=" + std::to_string(SHORTFALL_MARGIN_PCT) +
              " budget=" + std::to_string(budget));
        check("SREP-07b", "and is the TIGHTEST whole point that does",
              SHORTFALL_MARGIN_PCT == tightest,
              "margin=" + std::to_string(SHORTFALL_MARGIN_PCT) +
              " tightest=" + std::to_string(tightest));
    }

    // --- SREP-08: the noise budget really is quiet, and 3 frames is enough --
    ...
    // interventions the budget pays for.
    {
        ...
        check("SREP-08a", "no window within the 3-frame noise budget is flagged",
              quiet);
        ...
        check("SREP-08b", "a window one frame past the budget is flagged",
              beyond.shortfall, got(beyond));
    }

    // --- SREP-09: the complaint that drove the retune -----------------------
    ...
    // "100%"; it must now be reported.
    {
        ...
        check("SREP-09", "a host at 91% of true rate is reported, not hidden",
              r.shortfall && r.achieved_pct == 91, got(r));
    }

    // --- SREP-10: multipliers are comparable on the same scale -------------
    ...
    // report that measured achieved against 1x would flag every fast-forward.
    {
        ...
        check("SREP-10a", "2x actually achieving 2x reports 200% and no shortfall",
              ok.requested_pct == 200 && ok.achieved_pct == 200 && !ok.shortfall,
              got(ok));
        check("SREP-10b", "2x achieving only 1x is a shortfall at 100%",
              short_.requested_pct == 200 && short_.achieved_pct == 100 &&
              short_.shortfall, got(short_));
    }

    // --- SREP-11: half speed is a first-class target -----------------------
    ...
    // and is not a shortfall.
    {
        ...
        check("SREP-11", "0.5x delivering half the refresh is on target",
              r.requested_pct == 50 && r.achieved_pct == 50 && !r.shortfall,
              got(r));
    }

    // --- SREP-12: a paused machine is "no measurement", never 0% -----------
    ...
    // "0%" there would be a false alarm on a deliberate state.
    {
        ...
        check("SREP-12a", "zero emulated frames yields no achieved figure",
              !r.achieved_valid, got(r));
        check("SREP-12b", "zero emulated frames is not reported as a shortfall",
              !r.shortfall, got(r));
        check("SREP-12c", "the requested figure survives a paused window",
              r.requested_pct == 100, got(r));
    }

    // --- SREP-13: an unknown frame period yields no measurement ------------
    ...
    // produce inf/NaN and a garbage label).
    {
        ...
        check("SREP-13a", "zero frame period yields no achieved figure",
              !z.achieved_valid && !z.shortfall, got(z));
        check("SREP-13b", "negative frame period yields no achieved figure",
              !n.achieved_valid && !n.shortfall, got(n));
    }

    // --- SREP-14: overachieving is reported, never clamped -----------------
    ...
    // the timer and the machine's period.
    {
        ...
        check("SREP-14", "achieved above the target is reported as-is",
              r.achieved_pct == 120 && !r.shortfall, got(r));
    }

    // --- SREP-15: the requested figure never depends on the measurement ----
    ...
    // fix must not regress.
    {
        ...
        check("SREP-15", "requested_pct tracks the multiplier in every case",
              a.requested_pct == 400 && b.requested_pct == 400,
              got(a) + " / " + got(b));
    }

    // --- SREP-16: shortfall implies a valid measurement --------------------
    ...
    // true while `achieved_pct` is meaningless.
    {
        ...
        check("SREP-16", "shortfall is never set without a valid measurement",
              implied);
    }

    // --- SREP-17: rate_from_window removes the coarse-timer error ----------
    ...
    // margin, so it is asserted, not assumed.
    {
        ...
        check("SREP-17d", "the un-normalised count is materially further off",
              summarize(47.0, NEXT50_MS, 1.0).achieved_pct <
              a_short - 3,
              std::to_string(summarize(47.0, NEXT50_MS, 1.0).achieved_pct));
    }

    // --- SREP-18: an unmeasurable window falls back, never divides by zero --
    {
        using speed_report::rate_from_window;
        check("SREP-18a", "a 1000 ms window returns the count unchanged",
              rate_from_window(49.0, 1000) == 49.0);
        check("SREP-18b", "a zero window returns the count, not inf/NaN",
              rate_from_window(49.0, 0) == 49.0);
        check("SREP-18c", "a negative window returns the count too",
