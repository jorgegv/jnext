...
:   Load a program. The format is detected from the extension: `.nex`, `.sna`,
    ...
    NEX. If its header asks for the file to be kept open (`file_handle` 1 or an
    address of 0x4000 or above), the program gets that handle and can read the
    extra bytes from its own file. If the header says `file_handle` 0, jnext
