...
:   Headless only: requires **\--headless**. Save a snapshot after a delay in
    ...
    machine *is* refused: `.sna` and `.szx` describe a 48K/128K/+3 Spectrum,
    so asking for either on a Next writes nothing, says why, and exits
    non-zero (see **JNEXT SNAPSHOTS**).
