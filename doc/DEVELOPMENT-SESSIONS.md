...
| 8/7     | 2h   | Issue build fixes (Debian 12 cstddef, nextboot.rom tracked, noexecstack); github-cli skill             |
...
| 12/7    | 5h   | Audio: beeper clicking = audio underruns (== issue ); paced emulation on the audio clock.              |
...
|         |      | 14 reviewed branches, profile-guided. Issue crash; 60a/60b; 13a→0 skips; v0.98.3 ChangeLog.            |
...
|         |      | videoint: NR 0x1E/0x1F = cvc + tilemap scroll latch; PR esxDOS chaining; Task 65 WONTs                 |
| 18/7    | 10h  | Task 77 input remaps (Tab=EXTEND, Esc=BREAK, Alt+E/G/C); Task 83 raw SDL_Joystick USB pads             |
|         |      | esxDOS syscall trace; NEXTEST DISK_FILEMAP streaming; cadence; SIGPIPE. v0.98.38->.45                  |
| 19/7    | 10h  | NR 0xB2 from Joystick, extended keys (membrane.vhd:253), mouse capture was ABSOLUTE                    |
|         |      | Task 93 build-matrix guard + CI; TODO.md -> GitHub issues. 8 merges, 5 REJECT. v0.98.52->.56           |
| 20/7    | 12h  | CLOSED: 9-chapter user guide + pandoc man page; docs-check gates every test run; Phase 9 done          |
|         |      | CI -> fedora:44 after a piped `make` target reported GREEN on a failing suite.. v0.98.71               |
| 21/7    | 4h   | self-contained macOS jnext.app: @rpath load-chain closure, codesign, otool-verified gate               |
|         |      | Task 21-01 unified build/<variant>-<config> dirs; every CI step via a make target. v0.98.72->.74       |
| 22-23/7 | 13h  | LoRes 128x96 shipped (91 VHDL-cited rows written BEFORE code); unified ULA+/ULA palette store          |
|         |      | per-run private SD clone - SIX review rounds, 3-5 worse than the original. v0.99.0 PUBLIC              |
| 23/7    | 3h   | Windows UTF-8 manifest; NR $15 per-line replay wired (had shipped with zero callers)                   |
|         |      | sprite_tie completion; // harness self-test gaps (stubs, no timeout). v0.99.1->.8                      |
| 24/7    | 7h   | ATIC ATAC CLOSED: = missing SD Nac gap byte before the 0xFE data token; stall; NEX                     |
|         |      | PRs ///. Tree-wide ZERO unit skips (5527/5527). filed. v0.99.9->.14                                    |
| 25/7    | 10h  | Ten issues + PR ( TX-1696 tilemap fetch splits); power/soft reset split; CMD9/10 gap                   |
|         |      | 28 MHz SRAM wait (~7% fast); /// ULAnext border; +6 more. v0.99.15->.28                                |
| 26/7    | 9h   | TX-1696 CLOSED: freeze = DMA stall gated on is_active, not dma_holds_bus; the fix then                 |
|         |      | exposed a per-frame im2_dma_delay reboot.; held on its branch. v0.99.29->.37                           |
| 27/7    | 11h  | CLOSED, Windows 7/8: Qt5 compat behind JNEXT_FORCE_QT5 + CI guard, WinHTTP/BCrypt, i686 leg            |
|         |      | matrix precision; night run // +. v0.99.38->.54                                                        |
| 28/7    | 13h  | ESP-01 branches 1-3.5: UartDevice attach seam, non-blocking TCP + address policy, AT engine            |
|         |      | Modularised into src/esp01 (off-thread DNS + design doc); matrix from unit-tests.conf. v0.99.55->.71   |
| 29/7    | 6h   | CLOSED with : --esp/--no-esp/--esp-allow, config keys, status-bar cell, man NETWORKING                 |
|         |      | RX pacing settled by measurement; VERIFIED LIVE against the real nx.nxtel.org BBS. v0.99.72->.74       |
| 30/7    | 14h  | NEX V1.3 loading screens + nexload2 palette handback; NextSync 1.2 verified end to end                 |
|         |      | Traceability citation extractor; mid-frame tilemap palette lane. v0.99.75->.89                         |
| 31/7    | 15h  | v1.0 issue chain cleared: Copper hc domain, matrix rows, nr_04 preserved across reset                  |
|         |      | phase 1: matrix classifier and section-scoped triage. v0.99.90->.107                                   |
| 1/8     | 16h  | landed: the traceability matrix becomes a generated, staleness-gated artifact                          |
|         |      | ESP-01 UDP for newt's SNTP; matrix no longer depends on who has the FPGA core. v0.99.108->.110         |
| 2/8     | 13h  | Developer guide published; Step Out (F8) was dead code; owner paths purged + linted                    |
|         |      | CREDITS.md and the man page name the contributors. v0.99.111->.114                                     |
| 3/8     | 4h   | --delayed-nmi headless NMI/DRIVE button press; traceability finds the FPGA tree from any worktree      |
|         |      | Worktrees relocated to ~/tmp/worktrees. v0.99.115->.117                                                |
