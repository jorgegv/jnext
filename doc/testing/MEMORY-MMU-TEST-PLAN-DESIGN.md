...
Measured on main 2026-04-20 post-Phase-2-D1 merge:

- **145 plan rows total** (N8E-05 split into 05a/05b + CON-12 split into 12a/12b in test — aggregated by matrix script per SUBLETTERS rule; 154 check()+skip() calls in test).
- **140 pass, 0 fail, 14 skip.**
...
- **Remaining 14 skips blocked by** Phase 2 branch D2 (Layer 2 read-port), plus 3 DivMmc-overlay rows (PRI-01/02/04) destined for integration tier, 2 altrom SRAM-arbiter overrides (ALT-08, ROM-09 — need full sram_pre_rdonly wiring), NR 0x12/0x13 shadow (integration tier), and Tilemap NR 0x1B read cycling (Phase 2 E).
