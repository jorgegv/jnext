...
| CMG-01     | the gate condition's **`boot_rom_enabled()` clause** | Next start with NO boot ROM (the `--load` case), nothing else | gate NOT entered: latch reads its power-on true, `mmu().config_mode()` stays false, and a write into the still-ROM slot 0 is DROPPED |
