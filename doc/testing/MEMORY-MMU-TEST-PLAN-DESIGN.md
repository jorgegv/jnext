...
| BOOT-SNAPSAVE-02 | `.szx` save round-trip via GUI/CLI               | Same; save as `.szx`                                                                                 | pass (Task 13b) — full CPU register set (both AF/BC/DE/HL sets, IX/IY/SP/PC/I/R/IFF/IM/halted), classic 128K/+3 paging (7FFD/1FFD), border, and every RAM bank up to the 112-bank/1792 KB MMU-addressable ceiling round-trip byte-for-byte (`mmu_test.cpp` BOOT-SNAPSAVE-02/02B/02C structural + `mmu_integration_test.cpp` SNAPSAVE-SZX-RT full pipeline). NextREG/video-layer/peripheral state does NOT round-trip — no standard zx-state block exists for it (see `SzxSaver` class doc-comment). |
