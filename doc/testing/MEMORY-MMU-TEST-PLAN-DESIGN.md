...
| BOOT-SNAPSAVE-02 | `.szx` save round-trip via GUI/CLI               | Same; save as `.szx`                                                                                 | pass (Task 13b) — full CPU register set (both AF/BC/DE/HL sets, IX/IY/SP/PC/I/R/IFF/IM/halted), classic 128K/+3 paging (7FFD/1FFD), border, and every RAM bank up to the **64-bank/1024 KB** ceiling round-trip byte-for-byte (`mmu_test.cpp` BOOT-SNAPSAVE-02/02B/02C structural + `mmu_integration_test.cpp` SNAPSAVE-SZX-RT/SNAPSAVE-SZX-RT-CEILING full pipeline). 64, not 112: 112 is jnext's own MMU-addressability ceiling (see the `.nex` row below) — the real, binding `.szx` ceiling is libspectrum 1.5.0's `szx.c:read_ramp_chunk`, which hard-rejects any `chPageNo > 63` for every machine ID (confirmed against the upstream source and the real `fuse` binary). An earlier cut of this saver used the wrong (112) number, silently producing files real FUSE rejects outright (`fuse: error: libspectrum: szx.c:read_ramp_chunk: unknown page number 64`) — caught by independent review, fixed, re-verified. NextREG/video-layer/peripheral state does NOT round-trip — no standard zx-state block exists for it (see `SzxSaver` class doc-comment). |
