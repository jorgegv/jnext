...
| ~~BOOT-FDC-03~~ | ~~NR 0x81 b3 (`fdc` clken) gates motor-on~~ | ~~NR 0x81 ← bit3=1; observe drive-motor LED state via NR introspection~~ | **RETIRED 2026-09-24 (GH #201).** The row's technical claim is **correct** — NR 0x81 bit 3 really does gate the motor, at the latch rather than at the output: `zxnext.vhd:3751-3753` reads `elsif nr_81_expbus_fdc = '0' then port_1ffd_mtr_n <= '1';  -- disk motor off`, so clearing the bit forces the motor signal off, and the port 0x1FFD bit 3 write at `:3757` is only reachable while the bit is set. It is retired because on the Issue 2 reference target **there is no motor for it to gate**: `gen_fdc_234` (`:1681-1689`) drives `o_BUS_P3_MTR_n <= '1'` unconditionally at `:1685`, ignoring `port_1ffd_mtr_n` entirely, and `zxnext_top_issue2.vhd:2468-2470` leaves that pin `=> open`. Only `gen_fdc_5` (`:1691`, board issue >= 3) forwards the latch to the pin at `:1695`. The row's stimulus has no counterpart either: there is no NR introspection of drive-motor state, and the latch is readable on Issue 2 only through the Multiface register readback (`:4312`), which is Multiface-suite territory. <br><br>**Correction of a correction:** the first revision of this note (commit `903d7251`) asserted the row was “factually wrong as written” because the motor was “ungated by NR 0x81”. That was itself wrong — it cited `:1695`, which is the Issue 5 branch the reference target does not take, and never read the latch process at `:3744-3760` where the gating actually happens. An absence was claimed from one signal path without searching the rest. |
