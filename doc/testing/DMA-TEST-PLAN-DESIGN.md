...
| ~~7.3~~ | ~~CE/WAIT mux bit~~ | ~~`R5_ce_wait_s <= cpu_d_i(4)`~~ | **RETIRED 2026-09-24 (GH #201)** — the bit has no hardware effect in the authoritative VHDL. `R5_ce_wait_s` is written at `dma.vhd:622`, reset at `:237` and `:644`, and read at exactly two places — `:341` and `:409` — **both of which are commented out in the VHDL source**. It is a write-only dead latch. jnext is VHDL-faithful and models no consequence for it, so the only assertion available would be a setter→getter round-trip on a field nothing reads: the tautology class `test/lint-assertions.sh` exists to reject. Revisit only if those two VHDL branches are ever re-enabled upstream. |
| ~~7.4~~ | ~~R5 defaults on reset~~ | ~~Both bits = 0~~ | **RETIRED 2026-09-24 (GH #201)** — split across its two bits, neither of which leaves anything to assert. `R5_auto_restart_s = 0` at reset is proven *behaviourally* by live row **7.2** (fresh DMA, run a block to completion, state is IDLE rather than TRANSFERRING — which is only true if auto-restart defaults off, `dma.vhd:238, :494`). `R5_ce_wait_s = 0` is the same dead latch as row 7.3. |
...
| ~~15.8~~ | ~~DMA cannot self-program~~ | ~~`port_dma_rd/wr` gated by `not dma_holds_bus`~~ | **RETIRED 2026-09-24 (GH #201)** — covered live by **REG-22-BUS** in `test/port/port_test.cpp`, which is where the behaviour belongs: the gate is at the port dispatcher (`zxnext.vhd:2768-2769`), not inside `Dma`. That row programs a 4-byte transfer on a live `Emulator`, steps one burst so `dma_holds_bus()` is true with bytes still pending, then asserts BOTH halves of the gate — an `OUT 0x6B,0xC3` (DMA RESET) is dropped (state stays TRANSFERRING) and an `IN 0x6B` returns 0xFF. The VHDL agrees on the intent, commenting the port at `:1780` with *“allow dma to program itself? ans: no”*. |
