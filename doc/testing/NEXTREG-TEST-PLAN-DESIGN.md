...
Measured on main 2026-04-20 post-Task-3 NextREG Phase 1 re-home:

- **66 plan rows total** (CLIP-09/10 added 2026-04-20 Phase-2-E).
- **Bare test** (`test/nextreg/nextreg_test.cpp`): 21 pass / 0 fail / **2 skip** (COP-02, COP-03 — cycle-accurate CPU+Copper bus arbitration requires a harness the bare surface cannot drive). Phase 1 of Task-3 re-homing (2026-04-20) converted 32 bare skip()s to source comments pointing at the covering test: RO-01/03/05/06 + RST-09 + CLIP-06/07 + PAL-01..06 + CFG-05 → `nextreg_integration_test.cpp` (passing); SEL-03/RO-02/04 + RW-01 + CLIP-01..05/08 + CFG-01/02 + PE-03/05 → `nextreg_integration_test.cpp` (tracked as skip, real feature gap); MMU-03/04 → `mmu_test.cpp` Cat3 P7F-01..15 + Cat1 MMU-13/14 (coupling covered via port-path round-trip); COP-04 → `copper_test.cpp` ARB-04 + MOV-02/07 (7-bit reg mask); SEL-05 → Z80N compliance suite (ED 91/92 NEXTREG opcodes).
- **Integration test** (`test/nextreg/nextreg_integration_test.cpp`): 55 pass / 0 fail / 14 skip out of 69 rows. PE-03 (NR 0x82 bit 6 gate on port 0x1F) newly tracked at integration tier 2026-04-20 with backlog note (Kempston handler in `src/core/emulator.cpp:1119` is unconditional — real feature gap).
- **Emulator Bug backlog items (live — Phase 2 targets)**: RO-02/SEL-03 (NR 0x00 RO enforcement — install read_handler returning 0x08 HWID_EMULATORS). RO-04 (seed regs_[0x0E]=0x03). PE-05 (seed regs_[0x89]=0x8F). PE-03 (install NR 0x82 bit 6 gate on port 0x001F handler). CLIP-01..05 (Layer2/Sprite/Ula public clip getters + `Emulator::ula()` accessor). CLIP-08 (NR 0x18 read_handler — combinatorial mux per zxnext.vhd:5947-5953). RW-01 (NR 0x07 packed read — speed FSM accessor). CFG-01 (NR 0x03 composed read — ULA retime state accessor). CFG-02 (NR 0x03 bit 3 dt_lock XOR).
- **Matrix-tooling gap:** `test/refresh-traceability-matrix.pl` scans the one `nextreg_test.cpp` file per subsystem; Phase 1 re-homed skip()s show as `missing` in the per-row matrix table for NextREG (43 rows). The covering test location is captured as a source comment on each re-homed row in `nextreg_test.cpp`. Backlog: extend script to (a) recognise sub-letter prefixes (CLIP-09a/09b) and (b) follow cross-file re-homing pointers. Tracked in TRACEABILITY-MATRIX.md.
