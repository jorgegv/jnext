...
| G2.PL-09 | Tie CLEAR: NR 0x34 leaves pattern_index — including pattern_index(7) — untouched; the stale bit is observable end-to-end (NR 0x75 inc reloads mirror bit 7 from pattern_index(7); a later tie-set sync would carry a wrong 128-byte half-pattern offset) | Tie=0; 0x303B ← 0x02 (pattern_index=0x0200, bit 7 = 0) | NR 0x34 ← 0x85; NR 0x75 write; set tie; NR 0x75 write | pattern_index still 0x0200 after NR 0x34; mirror = 0x06 (bit 7 = 0) after first inc; after tie-set inc mirror = 0x07 and pattern_index = 0x0700 (low half) | 728–741, 603–605, 733–734 (GH #74) |
| G2.PL-10 | Tie SET: NR 0x34 re-bases the FULL pattern_index per `mirror(5:0) & mirror(7) & "0000000"` — bit 7 included; the next NR 0x75 inc reads pattern_index(7) back into mirror bit 7 | Tie=1 | NR 0x34 ← 0xA0; NR 0x75 write | pattern_index = 0x2080 after NR 0x34; mirror = 0xA1 after inc; pattern_index = 0x2180 | 733–734, 603–605 (GH #74) |
...
**G04.PSL-NR4B-01 — closed (GH #256).** The NR 0x4B per-scanline row was
implemented as `PSCAN-G04-02` in the COMPOSITOR plan's companion suite
`test/compositor/compositor_integration_test.cpp`: it needs a full `Emulator`
with a running Copper, which this bare suite cannot build. It drives a Copper
`MOVE NR 0x4B` at cvc 60 and requires the sprite hidden above the split row and
visible from it. The same group pins the sprite engine's other render controls
per row — the NR 0x19 clip window and NR 0x15 bits 1, 5 and 6 (`PLRS-SPR-01..04`)
— all read at their end-of-frame value before GH #256. `SpriteEngine` now takes
them from `snapshot_control_for_line()` (captured by `Emulator::on_scanline` on
the row the attribute change-log tags a write with). The transparency index and
bit 6 filter the pixels the FSM writes into the NEXT row's line buffer
(`sprites.vhd:971-972`), so jnext applies them one row earlier than the
hardware would — exactly as it already does for the attributes they filter,
which keeps the lane coherent; the clip, bit 5 and bit 1 gate the buffer as it
is read out (`sprites.vhd:1037-1067`), which the row model matches.
