...
| G04.PSL-NR4B-01 | NR 0x4B (sprite transparent index) write logged + replayed per scanline                                            | start_frame; line=80 NR 0x4B ← 0x40                                                                                                                                  | rows 0..79 use 0xE3 (default sprite_transparent_index); rows 80..end use 0x40. stub — `Sprites::sprite_transparent_index_` is a scalar today; no per-scanline change-log. Re-homed 2026-04-28 from `compositor_test.cpp` (PSCAN-G04-02) per cross-bucket G04 ownership; NR 0x4B is owned by Sprites — VHDL `zxnext.vhd:5016, 1190`, `sprites.vhd` pixel engine                                                                                                                              |
