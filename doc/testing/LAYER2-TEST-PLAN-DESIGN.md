...
| G7-17 | port 0x123B write with bit 4 = 1 latches offset, leaves enable/wr_en/rd_en/segment unchanged | reset, port 0x123B ← 0x12 (bit 4 = 1, bits 2:0 = 010), then read-back | `port_123b_layer2_offset = "010"`; `enable`, `wr_en`, `rd_en`, `map_shadow` all unchanged from prior values (Wave-2 G92 closed: `Mmu::set_l2_port` bifurcates on bit 4) | zxnext.vhd:3914-3923 |
| G7-18 | port 0x123B bit 3 = 1 routes CPU writes/reads to `nr_13_layer2_shadow_bank` | NR 0x12 = 0x08, NR 0x13 = 0x0B, port 0x123B ← 0x09 (bit 0 = 1, bit 3 = 1), write marker byte at segment-0 offset; toggle bit 3 = 0 and bit 2 = 1 to read | first read returns shadow-bank marker; write landed in NR 0x13 page (0x0B), not NR 0x12 page (0x08) (Wave-2 G144 closed: `Mmu::l2_map_shadow_` + `set_l2_shadow_bank`) | zxnext.vhd:2968 |
| G7-19 | port 0x123B read returns the formatted control word, not PortDispatch default 0xFF | reset; port 0x123B ← 0x49 (bit 0 = 1, bit 3 = 1, bits 7:6 = 01); IN A,(0x123B) | read returns `0x49` (matches written control word per G7-16 formatter); NOT `0xFF` (Wave-2 G145 closed: read handler in emulator.cpp + `Mmu::l2_port_readback`) | zxnext.vhd:3933, emulator.cpp |
...
| ~~G9-04~~ | ~~Scroll X with wide branch NOT fired~~ | — | **RETIRED 2026-09-24 (GH #201)** — this is exactly `G3-12`'s claim. The "wide branch" is the wide-mode-only wrap correction `x(8 downto 6) <= x_pre(8 downto 6) + "011"` at `layer2.vhd:152`; `G3-12` runs in WIDE mode (`setup_wide(200)`) with `scroll_x = 100`, so `x_pre = 100 < 320` leaves the branch guard satisfied and the correction unapplied, and the row only passes if column 0 sources source-column 100. Re-running the same stimulus under a second ID adds no signal. Verified: `G3-12` is a live `check()` at `test/layer2/layer2_test.cpp:640`. No `check()` row exists for G9-04. | — | — |
...
| ~~L2-G17-01~~ | ~~Parallax.nex side-by-side duplication root cause (post-LoRes)~~ | **RETIRED 2026-09-24 (GH #201)** — an investigation-class placeholder, never an assertion (its own Expected column said "placeholder; concrete oracle to be authored"). The investigation CLOSED 2026-05-01: the "two copies" symptom was three coordinate-space defects cancelling (G164v2, commits cde0a45 / f5bed99 / 8fcb345 / 460fac6). Live coverage, by real row ID and file: `PSCAN-VBLANK-COALESCE-01` (`test/compositor/compositor_integration_test.cpp:498`), `PSCAN-G10-01`..`PSCAN-G10-05` (`test/compositor/compositor_test.cpp:4052-4400`), this suite's per-scanline change-log rows `G10-01`..`G10-05`, `G10-G05-01a`..`G10-G05-02` and `G10-G09-01a`..`G10-G09-02`, and the pinned 100-frame parallax-demo screenshot baseline. No `check()` row exists. | — |
