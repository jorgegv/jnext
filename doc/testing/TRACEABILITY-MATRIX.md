...
Rows the sections above carry: **4643**. Distinct row IDs recorded anywhere in this document (every table, including "Extra coverage"): **4375**. Rows the 107 suites declared in `test/unit-tests.conf` run live: **7661**.
...
These 66 suites (3474 live rows) have no VHDL-derived plan row to map, so they have no section here. They are still declared, counted and run; their runtime view is `test/SUBSYSTEM-TESTS-STATUS.md`.
...
| `debugger_disasm_copy_test` | 27 | disassembly-panel selection and clipboard text (host GUI, GH #21); the disassembler it copies is traced in `## Z80N` |
