...
Rows the sections above carry: **4444**. Distinct row IDs recorded anywhere in this document (every table, including "Extra coverage"): **4233**. Rows the 102 suites declared in `test/unit-tests.conf` run live: **7286**.
...
These 61 suites (3242 live rows) have no VHDL-derived plan row to map, so they have no section here. They are still declared, counted and run; their runtime view is `test/SUBSYSTEM-TESTS-STATUS.md`.
...
| `emulator_boot_test` | 47 | host cold-boot choreography (GH #40 contract, no VHDL oracle) |
...
| `rzx_menu_test` | 7 | RZX File-menu error dialogs (GUI), no core counterpart |
