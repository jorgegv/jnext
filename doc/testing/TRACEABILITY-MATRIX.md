...
Rows the sections above carry: **4737**. Distinct row IDs recorded anywhere in this document (every table, including "Extra coverage"): **4469**. Rows the 110 suites declared in `test/unit-tests.conf` run live: **7945**.
...
These 68 suites (3664 live rows) have no VHDL-derived plan row to map, so they have no section here. They are still declared, counted and run; their runtime view is `test/SUBSYSTEM-TESTS-STATUS.md`.
