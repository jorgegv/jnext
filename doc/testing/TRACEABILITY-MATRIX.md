...
Rows the sections above carry: **4801**. Distinct row IDs recorded anywhere in this document (every table, including "Extra coverage"): **4504**. Rows the 114 suites declared in `test/unit-tests.conf` run live: **8269**.
...
These 71 suites (3885 live rows) have no VHDL-derived plan row to map, so they have no section here. They are still declared, counted and run; their runtime view is `test/SUBSYSTEM-TESTS-STATUS.md`.
