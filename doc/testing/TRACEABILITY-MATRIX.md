...
Rows the sections above carry: **4818**. Distinct row IDs recorded anywhere in this document (every table, including "Extra coverage"): **4510**. Rows the 116 suites declared in `test/unit-tests.conf` run live: **8727**.
...
These 73 suites (4202 live rows) have no VHDL-derived plan row to map, so they have no section here. They are still declared, counted and run; their runtime view is `test/SUBSYSTEM-TESTS-STATUS.md`.
