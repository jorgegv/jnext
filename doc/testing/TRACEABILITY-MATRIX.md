...
Rows the sections above carry: **4261**. Distinct row IDs recorded anywhere in this document (every table, including "Extra coverage"): **4074**. Rows the 96 suites declared in `test/unit-tests.conf` run live: **6854**.
...
These 55 suites (2988 live rows) have no VHDL-derived plan row to map, so they have no section here. They are still declared, counted and run; their runtime view is `test/SUBSYSTEM-TESTS-STATUS.md`.
...
| `resume_step_off_test` | 16 | debugger resume/step-off execution control (GH #221, jnext-internal); the T80N core has no debugger |
...
| `debugger_menu_test` | 20 | debugger menu reachability (host GUI) |
