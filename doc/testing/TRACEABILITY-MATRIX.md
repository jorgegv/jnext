...
Rows the sections above carry: **4128**. Distinct row IDs recorded anywhere in this document (every table, including "Extra coverage"): **3947**. Rows the 91 suites declared in `test/unit-tests.conf` run live: **6651**.
...
These 50 suites (2912 live rows) have no VHDL-derived plan row to map, so they have no section here. They are still declared, counted and run; their runtime view is `test/SUBSYSTEM-TESTS-STATUS.md`.
...
| `step_out_test` | 41 | debugger Step Out execution control (jnext-internal); the T80N core has no debugger |
