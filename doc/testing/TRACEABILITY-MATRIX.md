...
Rows the sections above carry: **4261**. Distinct row IDs recorded anywhere in this document (every table, including "Extra coverage"): **4074**. Rows the 98 suites declared in `test/unit-tests.conf` run live: **6958**.
...
These 57 suites (3092 live rows) have no VHDL-derived plan row to map, so they have no section here. They are still declared, counted and run; their runtime view is `test/SUBSYSTEM-TESTS-STATUS.md`.
