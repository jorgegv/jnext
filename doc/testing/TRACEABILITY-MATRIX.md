...
Rows the sections above carry: **4363**. Distinct row IDs recorded anywhere in this document (every table, including "Extra coverage"): **4176**. Rows the 100 suites declared in `test/unit-tests.conf` run live: **7139**.
...
These 59 suites (3171 live rows) have no VHDL-derived plan row to map, so they have no section here. They are still declared, counted and run; their runtime view is `test/SUBSYSTEM-TESTS-STATUS.md`.
...
| `esp_wiring_test` | 98 | jnext host ESP policy/visibility/wiring, no core counterpart |
