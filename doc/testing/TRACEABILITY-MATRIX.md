...
Rows the sections above carry: **4751**. Distinct row IDs recorded anywhere in this document (every table, including "Extra coverage"): **4483**. Rows the 111 suites declared in `test/unit-tests.conf` run live: **8009**.
...
These 69 suites (3714 live rows) have no VHDL-derived plan row to map, so they have no section here. They are still declared, counted and run; their runtime view is `test/SUBSYSTEM-TESTS-STATUS.md`.
