...
Rows the sections above carry: **4751**. Distinct row IDs recorded anywhere in this document (every table, including "Extra coverage"): **4483**. Rows the 111 suites declared in `test/unit-tests.conf` run live: **8008**.
...
These 69 suites (3713 live rows) have no VHDL-derived plan row to map, so they have no section here. They are still declared, counted and run; their runtime view is `test/SUBSYSTEM-TESTS-STATUS.md`.
...
| `warm_start_test` | 38 | warm-start cache file format and invalidation keys (GH #234, jnext-internal); the residency rows assert what the FIRMWARE leaves, which the FPGA core does not specify |
