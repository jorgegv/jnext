...
These 65 suites (3446 live rows) have no VHDL-derived plan row to map, so they have no section here. They are still declared, counted and run; their runtime view is `test/SUBSYSTEM-TESTS-STATUS.md`.
...
| `emulator_boot_test` | 66 | host cold-boot choreography (GH #40 contract, no VHDL oracle) |
