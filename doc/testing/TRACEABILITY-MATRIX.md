...
Rows the sections above carry: **4444**. Distinct row IDs recorded anywhere in this document (every table, including "Extra coverage"): **4233**. Rows the 105 suites declared in `test/unit-tests.conf` run live: **7396**.
...
These 64 suites (3352 live rows) have no VHDL-derived plan row to map, so they have no section here. They are still declared, counted and run; their runtime view is `test/SUBSYSTEM-TESTS-STATUS.md`.
...
| `nex_loader_test` | 135 | NEX file-format spec (host loader), no core counterpart |
...
| `tzx_loader_test` | 64 | TZX container format, libspectrum TZX reader as oracle (host loader), no core counterpart |
