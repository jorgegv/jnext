...
| Z80-02 | Z80 accepts NMI, PC vectors to 0x0066 | zxnext.vhd:1841, :2168; t80n.vhd:1716-1718, 1765-1767 | pass — `test/nmi/nmi_test.cpp` group `Z80` (GH #201). DivMMC button strobe through the real FSM; the CPU is parked on a `JP 0xC000` self-loop so the pushed PC is exact. Asserts the whole acknowledge, not just the vector: PC = 0x0066, the interrupted PC written to the stack, SP -= 2, IFF1 cleared, and RETN restoring IFF1 from IFF2 and returning. Run with TWO stimuli (review strengthening): IFF1=1/IFF2=1 pins that the acknowledge clears IFF1, and IFF1=0/IFF2=1 pins that it leaves IFF2 standing — `t80n.vhd:1765-1767` writes `IntE_FF1` alone, and a single IFF1=IFF2=1 run cannot separate that from an acknowledge clearing both |
| Z80-03 | Reset clears both NmiSource state and Z80 NMI line | zxnext.vhd:2098-2101, :2154-2155, :2168 | pass — `test/nmi/nmi_test.cpp` group `Z80` (GH #201). Latch the DivMMC request and hand the CPU a pending NMI, then `soft_reset()`: the FSM is back in IDLE, all three request latches are clear, `nmi_generate_n` is released, AND stepping the re-parked CPU never reaches the vector — the half a NmiSource-only check cannot see (NMI-RST-02 covers the latches alone) |
| ~~Z80-04~~ | ~~NMIACK_LSB / NMIACK_MSB latch PC into nr_c2/c3 (Z80N command cross-link)~~ | ~~zxnext.vhd:2050-2085, 6232-6236~~ | **RETIRED 2026-09-24** (GH #201) — covered elsewhere, under the primary owner's IDs. RE-HOMED 2026-04-28 to the CTC+Interrupts plan, whose **NR-C2-01** and **NR-C3-01** both pass in `test/ctc_interrupts/ctc_interrupts_test.cpp` (NR 0xC2/0xC3 mirror the NMI return-address LSB/MSB, `zxnext.vhd:2050-2085`, `:6232`/`:6236`). Keeping the ID listed here only republished it as `missing` while the behaviour was asserted and green under another name. No `check()` row exists in `nmi_test`. |
...
| Z80 — Z80 drive + integration | 3 | request_nmi, PC=0x0066 + full acknowledge, reset-clear (Z80-04 RE-HOMED to the CTC plan 2026-04-28, RETIRED 2026-09-24) |
...
**All three are RETIRED as of 2026-09-24 (GH #201), and the section is now
empty of live rows.** None of them was ever a unit row in waiting: each
names a whole-NextZXOS-boot outcome with no bare-class surface to drive, and
each is proven at the regression tier by a named row of
`test/00regression/regression_tests.conf` (TEST-TAXONOMY.md Layer 1).

| ID | Description | VHDL file:line |
|----|-------------|----------------|
| ~~BOOT-LOOP-01~~ | ~~NextZXOS RAM-test outer loop (208 passes × 112 banks over ~15 s)~~ | **RETIRED 2026-09-24** (GH #201) — covered at the REGRESSION tier, not the unit tier, and it cannot be anything else: the claim is about a full NextZXOS boot, which has no bare-class surface to drive. `boot-nextzxos-welcome` in `test/00regression/regression_tests.conf` is the row that proves it — TEST-TAXONOMY.md **Layer 1** (full-OS SD boot: invocation has an SD card and no `--load`). Listing the ID here republished it as `missing` on every generator run while the behaviour was green; a unit row asserting it is impossible, so the honest record is this one. No `check()`/`skip()` row exists, and none can. |
| ~~BOOT-LOGO-01~~ | ~~NextZXOS loader logo + 4-entry log render~~ | **RETIRED 2026-09-24** (GH #201) — covered at the REGRESSION tier, not the unit tier, and it cannot be anything else: the claim is about a full NextZXOS boot, which has no bare-class surface to drive. `boot-nextzxos-splash` in `test/00regression/regression_tests.conf` is the row that proves it — TEST-TAXONOMY.md **Layer 1** (full-OS SD boot: invocation has an SD card and no `--load`). Listing the ID here republished it as `missing` on every generator run while the behaviour was green; a unit row asserting it is impossible, so the honest record is this one. No `check()`/`skip()` row exists, and none can. |
| ~~BOOT-DOT-01~~ | ~~NextZXOS BASIC + dot-command surface~~ | **RETIRED 2026-09-24** (GH #201) — covered at the REGRESSION tier, not the unit tier, and it cannot be anything else: the claim is about a full NextZXOS boot, which has no bare-class surface to drive. `boot-nextzxos-dotls` in `test/00regression/regression_tests.conf` is the row that proves it — TEST-TAXONOMY.md **Layer 1** (full-OS SD boot: invocation has an SD card and no `--load`). Listing the ID here republished it as `missing` on every generator run while the behaviour was green; a unit row asserting it is impossible, so the honest record is this one. No `check()`/`skip()` row exists, and none can. |
...
NMI Source Pipeline plan (`doc/testing/NMI-PIPELINE-TEST-PLAN-DESIGN.md`) closed end-to-end 2026-04-24d (per `project_session_handover_20260424d_eod.md` — Phase 1 scaffold + Wave A NR 0x02 + Wave B HK/DIS/CLR + Wave C gate registers + Wave E NMI-activated DMA delay all landed; 5 real emulator bugs fixed). The plan was reopened by Task 7 r1 + r2 (2026-04-24/25) which added skip rows for the residual gaps it surfaced — specifically G87/G88 (NMIACK PC-capture cross-link to NR 0xC2/0xC3 and CTC owner), G152/G153 (host F-key dispatch + NR 0x02 reset_type FSM bits 1:0), and G162 (iotrap strobe + Multiface port 0x2FFD/0x3FFD trap, parked until `MULTIFACE-TEST-PLAN-DESIGN.md` is authored). Companion integration suite `test/nmi/nmi_integration_test.cpp` runs the full button/software-NMI chain through a real `Emulator` fixture (NmiSource → arbiter strobe → DivMmc::set_button_nmi → cpu request_nmi → PC=0x0066 → automap → RETN clear); its 4 skips track the same G152 host-hotkey wiring debt at the GUI level. `nmi_test.cpp` runs at `77 / 77 pass / 0 fail / 0 skip` (Z80-04 re-homed to the CTC plan 2026-04-28 — duplicate cross-link with NR-C2-01/NR-C3-01 — and retired from this plan 2026-09-24) and `nmi_integration_test.cpp` at `10 / 10 / 0 / 0`. GH #201 (2026-09-23) added the 16 rows that closed the state machine itself: FSM-01..06 (IDLE → FETCH → HOLD → END → IDLE, the M1+MREQ-at-0x0066 gate, the consumer hold, END as a real one-tick state and its clearing of all three latches, and the config_mode force-back to IDLE), NMI-ARB-01..04 (first-come-first-served MF > DivMMC > ExpBus plus the `mf_is_active` block), NMI-RST-02/03 (split out of the NMI-RST-01 aggregate, whose own comment asked for it), EXPBUS-01..03 and Z80-01. A second GH #201 pass (2026-09-24) added the last two
rows of the Z80 group — **Z80-02** (the whole NMI acknowledge, not just the
vector: pushed PC, SP -= 2, IFF1 cleared, RETN restoring it) and **Z80-03**
(a reset drops the pipeline AND the CPU's pending NMI) — and retired the four
rows of this plan the matrix had been publishing as `missing` while their
behaviour was green elsewhere: **Z80-04** (owned by the CTC plan's NR-C2-01 /
NR-C3-01) and **BOOT-LOOP-01 / BOOT-LOGO-01 / BOOT-DOT-01** (regression tier,
TEST-TAXONOMY.md Layer 1). EXPBUS-02/03 are written to `zxnext.vhd:2091`, which has NO debounce term — NR 0x81 bit 5 is an extra term of `nmi_generate_n` at `:2166`, so it decides whether /NMI stays low after the FSM leaves IDLE/FETCH, not whether the bus can raise an NMI at all. The plan's "delayed assert (debounce path stubbed)" phrasing for those two rows does not match the VHDL and is superseded by this note.
