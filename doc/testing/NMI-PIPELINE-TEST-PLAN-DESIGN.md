...
| Z80-02 | Z80 accepts NMI, PC vectors to 0x0066 | zxnext.vhd:1841, :2168; t80n.vhd:1716-1718, 1765-1767 | pass — `test/nmi/nmi_test.cpp` group `Z80` (GH #201). DivMMC button strobe through the real FSM; the CPU is parked on a `JP 0xC000` self-loop so the pushed PC is exact. Asserts the whole acknowledge, not just the vector: PC = 0x0066, the interrupted PC written to the stack, SP -= 2, IFF1 cleared, and RETN restoring IFF1 and returning. IFF2 is deliberately not asserted — `t80n.vhd:1765-1767` writes only `IntE_FF1`, so with IFF1 = IFF2 = 1 the "copy" and "leave alone" readings are indistinguishable |
