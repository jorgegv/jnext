...
**Update (2026-09-24, GH #201):** the three rows above were *described* as
retired but never *struck*, so the generator went on emitting them as
`missing` — a claim of three open rows that nobody held. Two are struck
now, with the live row each is covered by named and read. The third,
**TIM-CYC-02, was not actually covered** and is a real row now. The
re-home note claimed a WAIT row would be redundant because "copper_test
Group 1-3 already exercises WAIT semantics"; those rows step
`Copper::execute` by hand and say nothing about how often the Emulator
calls it, which is the entire content of the row. The GH181-HCULA rows DO
measure a WAIT at the Emulator tier but bracket it to `kSlack = 16` raw
pixels — two instructions — so a Copper stepping once per instruction
passes them. Both facts were established by mutation, not by reading.
...
| WAI-03 | hpos=63 max — threshold WRAPS     | Instr[0]=WAIT hpos=63 vpos=0 (`0xFE00`); cvc=0   | hcount stepped 0,1,…                                                | The threshold add is 9-bit UNSIGNED (`6 bits & "000"` is 9 wide; `numeric_std` `"+"(UNSIGNED,NATURAL)` returns the LEFT operand's width), so 504+12 = 516 **wraps to 4**. addr stays 0 for hcount<4 and advances at hcount=4. (GH #182 — this row previously asserted the untruncated "never fires" reading.) | `copper.vhd:94`; `copper.vhd:35` |
| WAI-03b| hpos=55 — last reachable threshold| Instr[0]=WAIT hpos=55 vpos=7; cvc=7              | hcount 451 then 452                                                 | threshold = 55*8+12 = 452, no wrap (452 < 512), and 452 ≤ `c_max_hc`=455 so a real `hc_ula` reaches it in the 456-tick Next/128K timings (the 448-tick 48K line already cannot). addr stays 0 at 451, advances at 452. | `copper.vhd:94`; `zxula_timing.vhd:196` |
| WAI-03c| hpos 56..62 — above `c_max_hc`    | Instr[0]=WAIT hpos=h vpos=3 for h=56..62; cvc=3  | full hcount sweep 0..455 for each h                                 | thresholds 460..508 fit the 9-bit domain (no wrap) but exceed `c_max_hc`=455, the largest value `hc_ula` takes — so they never fire on hardware either. addr stays 0 for every h. | `copper.vhd:94`; `copper.vhd:35`; `zxula_timing.vhd:196`, `:427-436` |
| WAI-03d| canonical HALT `0xFFFF` stays dead| Instr[0]=`0xFFFF` (hpos=63, vpos=511), Instr[1]=MOVE NR 0x40 | full cvc sweep 0..319 × hcount ∈ {0,3,4,5,12,100,452,455} | After WAI-03 the hpos half of `0xFFFF` IS satisfiable (threshold wraps to 4), so HALT's deadness rests entirely on vpos=511 > `c_max_vc`. addr stays 0 and the MOVE never fires. This is the encoding real programs ship — `show512.nex`'s only hpos=63 word is this HALT. | `copper.vhd:94`; `copper.vhd:35`; `zxula_timing.vhd:196` |
...
| ~~TIM-CYC-01~~ | ~~Copper MOVE burst rate is per 28 MHz cycle, not per Z80 instr~~ | — | — | **RETIRED 2026-09-24 (GH #201)** — covered under a different ID, at the tier that can see it. `G117-MPC-01` (`test/copper/copper_integration_test.cpp:165`) programs 16 MOVEs to NR 0x14 with values 0x10..0x1F, starts the Copper on a mode 0->1 edge and calls `Emulator::execute_single_instruction()` three times; the row passes only if NR 0x14 ends at 0x1F, i.e. all 16 retired. One step per instruction gives 0x12. Read before retiring, and measured: the pre-G117 scheduler (one `Copper::execute` per instruction) fails it at 0x10. The bare-`Copper` harness this table belongs to cannot express the row at all — it has no instruction cadence to be wrong about. No `check()` row exists under this ID. — `device/copper.vhd:54-119`; `zxnext.vhd:3944,3950` |
| TIM-CYC-02 | Copper WAIT advances per 28 MHz cycle (boundary detection) | mode=`01`, Instr[0]=WAIT already satisfied for the CURRENT (hc_ula, cvc), Instr[1..8]=`MOVE 0x14, 0x10+i`, Instr[9]=HALT | one `Emulator::execute_single_instruction()` | NR 0x14 = 0x17 — the WAIT costs ONE 28 MHz cycle (`copper.vhd:92-98`, no `copper_dout_s` stall) and the eight MOVEs cost two each (`:100-108`, `:87-89`), so all nine retire inside the 32 master cycles of one 3.5 MHz instruction. **LIVE since GH #201 (2026-09-24)**, re-homed to `copper_integration_test.cpp` next to G117-MPC-01. It is NOT a duplicate of that row: a mutation that lets a satisfied WAIT consume the rest of its instruction window leaves G117-MPC-01 green and fails this one. The row establishes "already satisfied" rather than assuming it — it walks the raster to a point with the WAIT's own cvc line current and hc_ula in [20,300], then programs the WAIT for that cvc; NR-port programming executes no Z80 instruction, so the raster does not move in between. An earlier draft that stepped until the burst appeared and then allowed one more instruction was NOT discriminative, and the mutation said so. — `device/copper.vhd:92-98, 100-108`; `zxnext.vhd:3944,3949-3950` |
...
| ~~ARB-G65-01~~ | ~~True tied-edge CPU+Copper write~~ | — | — | **RETIRED 2026-09-24 (GH #201)** — covered under a different ID, and additionally superseded by a standing scope decision. `G65-PRI-01` (`test/copper/copper_integration_test.cpp:247`) puts a Copper `MOVE NR 0x14, 0x55` and a Z80N `NEXTREG 0x14, 0xAA` inside the SAME instruction window and requires the visible value afterwards to be 0xAA — the VHDL outcome at `zxnext.vhd:4769,4775-4777` (Copper's write fires first, the CPU request is held over and retires next). Read before retiring. The remaining difference — that jnext serialises the two writers instead of arbitrating a shared `nr_wr_*` bus on a 28 MHz edge — is the accepted modelling limitation recorded as WONT in `doc/design/EMULATOR-DESIGN-PLAN.md` ("Model cycle-accurate CPU/Copper NR write priority", resolved as option (a), 2026-07-17), gated on a cycle-accurate refactor that the same review ruled out. ARB-01/02/03 satisfy the VHDL outcome by ordering the stimulus manually under the same decision. No `check()` row exists under this ID. — `zxnext.vhd:4769, 4775-4777` |
...
| EDG-02 | Empty program (first slot WAIT impossible)| mode=`01`, Instr[0]=WAIT vpos=500 hpos=63 (vpos 500 > `c_max_vc`, so unreachable — the hpos plays no part, see WAI-03)| run 5 frames                    | addr stays 0; no Copper requests.                                                                                                          | `copper.vhd:92-96` |
...
## GH #181 append (2026-07-30) — WAIT `hpos` is compared against `hc_ula`, not master cycles

`copper.vhd:94` compares the WAIT threshold against `hcount_i`, and
`hcount_i` **is** `hc_ula`, with `vcount_i` **is** `cvc` — no conversion:
...
`hc_ula` is a **7 MHz per-PIXEL** counter (`zxula_timing.vhd:427-438`,
clocked on `i_CLK_7`), reset when the raw frame counter `hc` equals
`ula_min_hactive = c_min_hactive - 12` (`:423-424`). The reset is
REGISTERED, so it lands one tick later and
...
Pre-fix `Emulator::tick_copper_for_master_cycles` passed the raw
**28 MHz master-cycle offset into the raw scanline, origin 0** — wrong
by 4x in scale AND by the 125-pixel origin. Every WAIT with a
non-trivial `hpos` fired about a quarter of the way into the line it
should have, up to a full row early. Fix: rebase the frame-relative
master-cycle position by `VideoTiming::hc_ula_zero_raw_hc()` pixels and
pass `line_mc >> 2`.

Rows (in `copper_integration_test`), all measured against a NOP sled
(8 raw pixels per instruction) on the 128K/Next 50 Hz slot
(`c_max_hc=455`, `c_min_hactive=136`, `c_min_vactive=64`):

| ID | Description | VHDL |
|----|-------------|------|
| GH181-HCULA-01 | ORIGIN: `WAIT(hpos=0, vpos=100)` (threshold 12) fires at `hc_ula==12`, i.e. raw hc `125+12 = 137` of raw line 164 — not at raw hc 3 (pre-fix: 12 master cycles from the raw line start) | copper.vhd:94; zxula_timing.vhd:423-436; zxnext.vhd:3949,6737 |
