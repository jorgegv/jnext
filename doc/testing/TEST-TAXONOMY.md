...
Since GH #250 the shim is armed automatically for every directly loaded NEX (until the next reset), so `--esxdos-stub` on the rows below is redundant but harmless. The unit rows `ESXN-01..10` in `test/esxdos_stub/esxdos_stub_test.cpp` pin that, with replies measured on real NextZXOS (`.nexload` of a probe on the distro SD image).
