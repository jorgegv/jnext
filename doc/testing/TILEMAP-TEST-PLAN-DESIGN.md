...
`transp_colour_i` (NR 0x4C) and `clip_*_i` (NR 0x1B) are compared against the
...
| TM-GH257-01 | NR 0x30 (scroll X) written mid-frame via the Copper — the scroll lane on the end-of-row convention | Map A column 1 = tile 2; Copper `WAIT 100; MOVE 0x30,0x08` | Column 0 shows tile 1 above `100+DISP_Y` and tile 2 (scrolled in) from it. `tilemap.vhd:309,345-347` re-read the live `tm_scroll_x_i` (`zxnext.vhd:4419`) at every character's `S_IDLE` |
| TM-GH257-02 | 50 Hz: a write below the displayed surface does not reach the last row | Copper `WAIT 230` (raw line 294 > 287); `MOVE 0x6E` (map B) | Every row, 255 included, shows map A. Guards the end-of-frame snapshot, which must not overwrite row 255 with the frame's final value (the GH #16 HUD leak) |
| TM-GH257-03 | 60 Hz: the last row is captured although no scanline event follows it | NR 0x05 = 0x04, one frame to commit; then Copper `WAIT 100; MOVE 0x6E` | Rows from `100+DISP_Y` (raw line 140) through 255 (raw line 263, the frame's last) show map B |
| TM-GH257-04 | GH #16 shape: an IM2 line-interrupt handler's NR 0x30 write | Line interrupt target 100 (fires at cvc 99, hc_ula 255 = raw hc 380); handler `NOP×5; NEXTREG 0x30,8` at 3.5 MHz | The write lands at raw line 164, hc ~50, before the row's first visible fetch (whc −8 = raw hc 97, `tilemap.vhd:227-229,264`): column 0 shows the scrolled tile from `100+DISP_Y` on. Pins the line interrupt's position and the tilemap lane together (`zxula_timing.vhd:423-436,560-583`) |
