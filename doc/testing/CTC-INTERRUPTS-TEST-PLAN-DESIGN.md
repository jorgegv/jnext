...
| PULSE-G89-01 | LDIRX samples INT/NMI between iterations | pass — G89 closed; `test/ctc_interrupts/ctc_interrupts_test.cpp` asserts LDIRX (ED B4) runs ONE iteration per execute() and rewinds PC by 2 while BC != 0 (`t80n_mcode.vhd:2095-2138`). The inter-iteration sample itself is `PULSE-G89-INT`: a frame /INT raised mid-block is serviced between iterations with BC intact |
| PULSE-G89-02 | LDDRX samples INT/NMI between iterations | pass — same closure and suite as PULSE-G89-01 (ED BC) |
| PULSE-G89-03 | LDPIRX samples INT/NMI between iterations | pass — same closure and suite as PULSE-G89-01 (ED B7) |
| PULSE-G89-04 | LDIRSCALE samples INT/NMI between iterations | pass — same closure and suite as PULSE-G89-01 (ED B6) |
...
|----|-------------|----------------|
| ~~IM2-G89-01~~ | ~~LDIRX samples INT/NMI between iterations~~ | **RETIRED 2026-09-24** (GH #201) — duplicate ID. This is `PULSE-G89-01` in Section 10 above under a second name, and that row is LIVE and passing in `test/ctc_interrupts/ctc_interrupts_test.cpp` (G89 closed: each call runs ONE iteration and rewinds PC by 2 while BC != 0, `t80n_mcode.vhd`). The inter-iteration INT sample the description names is additionally pinned by `PULSE-G89-INT`, which raises a frame /INT mid-block and proves it is serviced between iterations with BC intact. No `check()` row exists under this ID. |
| ~~IM2-G89-02~~ | ~~LDDRX samples INT/NMI between iterations~~ | **RETIRED 2026-09-24** (GH #201) — duplicate ID. This is `PULSE-G89-02` in Section 10 above under a second name, and that row is LIVE and passing in `test/ctc_interrupts/ctc_interrupts_test.cpp` (G89 closed: each call runs ONE iteration and rewinds PC by 2 while BC != 0, `t80n_mcode.vhd`). The inter-iteration INT sample the description names is additionally pinned by `PULSE-G89-INT`, which raises a frame /INT mid-block and proves it is serviced between iterations with BC intact. No `check()` row exists under this ID. |
| ~~IM2-G89-03~~ | ~~LDPIRX samples INT/NMI between iterations~~ | **RETIRED 2026-09-24** (GH #201) — duplicate ID. This is `PULSE-G89-03` in Section 10 above under a second name, and that row is LIVE and passing in `test/ctc_interrupts/ctc_interrupts_test.cpp` (G89 closed: each call runs ONE iteration and rewinds PC by 2 while BC != 0, `t80n_mcode.vhd`). The inter-iteration INT sample the description names is additionally pinned by `PULSE-G89-INT`, which raises a frame /INT mid-block and proves it is serviced between iterations with BC intact. No `check()` row exists under this ID. |
| ~~IM2-G89-04~~ | ~~LDIRSCALE samples INT/NMI between iterations~~ | **RETIRED 2026-09-24** (GH #201) — duplicate ID. This is `PULSE-G89-04` in Section 10 above under a second name, and that row is LIVE and passing in `test/ctc_interrupts/ctc_interrupts_test.cpp` (G89 closed: each call runs ONE iteration and rewinds PC by 2 while BC != 0, `t80n_mcode.vhd`). The inter-iteration INT sample the description names is additionally pinned by `PULSE-G89-INT`, which raises a frame /INT mid-block and proves it is serviced between iterations with BC intact. No `check()` row exists under this ID. |
