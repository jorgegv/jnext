...
| REG-03a  | 0x243B read = reset value            | Power-on, no OUT to 0x243B yet  | IN 0x243B                                             | Returns 0x24, the `nr_register` reset value — not floating.  | `zxnext.vhd:4594-4596`, `:4603`          |
| REG-03b  | 0x243B read = selected register      | OUT 0x243B ← 0x1F               | IN 0x243B                                             | Returns 0x1F; the port is machine-driven, not floating bus.  | `zxnext.vhd:4603`, `:2818`, `:2804`      |
| REG-03c  | 0x243B selection survives ISR save/restore | Reg 0x07 selected by caller | IN 0x243B → OUT 0x243B ← other → OUT 0x243B ← saved   | Caller's selection restored; IN 0x253B still reads NR 0x07.  | `zxnext.vhd:4603`, `:2818`, `:2804`      |
...
| REG-17   | UART 0x133B accepted via TX-status   | UART subsystem                  | OUT 0x133B ← 'Z'                                      | Routes to UART TX-status window (cpu_a(15:11)="00011" per VHDL decode). | `zxnext.vhd:2639`                        |
...
| PR-DECL-01 | `decline_write()` state does not leak across a nested dispatch (G146 follow-up, 2026-07-14) | Outer handler (0xFFFF/0x1234) nests `write(0x00FF)` whose ONLY handler declines with no fallback; a less-specific handler (0x00FF/0x0034) also matches the outer address. Models DMA `write_io` (emulator.cpp ~5145) bursting to a gate-off SD2 port from inside an OUT. | Outer OUT dispatched exactly once — the dropped nested write resets the declined flag, so the outer frame does NOT spuriously fall through to the less-specific handler (double dispatch, forbidden by the one-hot decode). | `port_dispatch.cpp` write() no-handler exit path; VHDL one-hot decode `zxnext.vhd:2696–2699` |
