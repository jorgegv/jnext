...
| G163| Line-int schedule not re-evaluated on mid-frame NR 0x22/0x23 | Emulator, VideoTiming            | A   | Y       | parallax.nex chained line-IRQ chain swallowed; 69% black L2 | L     | High     |
...
### G163. Line interrupt schedule does not re-evaluate on mid-frame NR 0x22 / NR 0x23 writes — CLOSED 2026-04-30
- **What**: VHDL `zxula_timing.vhd:577` fires the line-int pulse every cycle when `(hc_ula==255 AND cvc==int_line_num)` — fully dynamic. jnext scheduled the line-int ONCE per frame in `Emulator::run_frame()`; the NR 0x22 / NR 0x23 write handlers updated `VideoTiming` state but did not re-schedule. Demos that chain line interrupts mid-frame (writing a new target into NR 0x23 from inside the line-IRQ handler) silently lost every chained re-arm — only the FIRST line-int per frame fired.
