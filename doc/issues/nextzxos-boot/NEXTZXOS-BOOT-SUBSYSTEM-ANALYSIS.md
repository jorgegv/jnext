...
| 22 | 2 fresh blind (memory + DivMMC skipped); enumeration-table mandate; pipelined | **0 class-a, 1 class-b (CPU) + 1 audit-finding DISMISSED as false-positive by reviewer (NMP) = 1 effective finding + NMP convergence** | CPU 152-row table + 1 class-b V22-IM2-01 (Im2Controller::on_reti missing im2_int_req latch clear per VHDL im2_peripheral.vhd:148,:175 im2_isr_serviced lock-step — companion to V21-IM2-01) + reviewer APPROVE-no-missed (13/13 state-variable symmetry between on_reti and step_state_machine_with_iei paths). **NMP 317-row table + 1 audit-claimed class-c V22-NMP-01 (NR 0xC2/0xC3 RO via NextReg-port writes) but reviewer REJECTED — audit misread VHDL :5601-5605 (vestigial commented duplicates) as the only write path; actual strobes nr_c2_we/nr_c3_we are ACTIVE at zxnext.vhd :4894-4895 (process A combinatorial decoder). Pre-fix jnext was VHDL-faithful; "fix" was a regression.** Fix-of-reviewer reverted bad RO-guard + flipped 4 tests to spec-correct writable contract + audit doc updated to mark V22-NMP-01 dismissed. Fix-reviewer APPROVE (VHDL re-confirmed independently + remediation surgical + adjacent re-audit clean). **NMP OFFICIALLY CONVERGED at Pass-22** → 3 of 4 subsystems converged. Only CPU remains active. **Trend**: P19=10, P20=9, P21=7, **P22=1** — sharp downtrend. Methodology validation: independent reviewer caught the audit's false-positive misread of vestigial VHDL comments — the enumeration-table + spot-check workflow correctly resisted enshrining a regression. ctest 38/38, FUSE 1356/1356, regression 33/0/0. |
| 23 | 1 fresh blind (CPU only — Memory + DivMMC + NMP all skipped); enumeration-table mandate | **0 class-a/b/c findings — CPU + Z80N + IM2 OFFICIALLY CONVERGED at Pass-23** | CPU 210-row table (156 canonical + 54 continuation, exceeding Pass-22's 152) + 0 findings + reviewer APPROVE-no-missed (15/15 spot-check + 9/9 key-claim verification: on_reti vs step_state_machine_with_iei symmetry, save/load schema coverage 122+ fields, CPU /INT polling symmetric (V19-IM2-04 + V20-IM2-01), Im2Level→DevIdx 14 slots, VHDL we-strobe vs vestiges clean, 31 Z80N opcodes verified, HALT+INT resume, CTC G119 unconditional callback, NMI vs IM2 precedence at z80_cpu.cpp:416-434). Cross-cutting families: multi-writer fan-out synchronized, WO-NR readback complete for NR 0xC0..0xCE, level-vs-pulse antipatterns absent, load_state shadow re-push correct. **ALL 4 BOOT-CRITICAL SUBSYSTEMS NOW CONVERGED**. Tests: ctest 38/38, FUSE 1356/1356, regression 33/0/0. |
| 24 | **CONVERGENCE PRESSURE TEST** — 4 fresh blind audits on all 4 already-converged subsystems (Memory re-walked after 10 inter-pass cycles; DivMMC, NMP, CPU re-walked under stricter post-P23 rigor); pipelined | **0 class-a, 0 class-b, 1 class-c (V24-DIVMMC-01 cosmetic CID MDT byte) + 2 class-d (V24-MEM-01 contention timing axis split; V24-DIVMMC-02 SDHC/SDSC mode) + 1 doc-NIT** | Memory 120/168-row table + 0 a/b/c + 1d V24-MEM-01 (reviewer disputed class-d→class-c reclassification, non-blocking — deferred outcome same) + reviewer APPROVE-WITH-NITS. DivMMC 112-row table + 1c V24-DIVMMC-01 (CMD10 CID MDT byte 14 0x65→0xA5 year 2022→2026 per SD spec § 5.2 Table 5-1; latent — TBBlue/FatFs don't read MDT) + 1d V24-DIVMMC-02 (SDHC/SDSC mode handling architectural per reviewer) + reviewer APPROVE-no-missed. NMP 347-row table (+30 vs P22 baseline) + 0 a/b/c + reviewer APPROVE-no-missed (differential P22→P24 ZERO NMP source delta — convergence by construction). CPU 220/260-row table + 0 a/b/c + reviewer APPROVE-no-missed (5/5 spot-check + differential P23→P24 ZERO source delta + 1 doc-only NIT row-count typo). **All 4 subsystems CONVERGENCE STABLE across the pressure test.** Tests: ctest 38/38, FUSE 1356/1356, regression 33/0/0. |
| 25 | **FINAL CONVERGENCE PRESSURE TEST** — 4 fresh blind audits, all 4 subsystems; pipelined | **0 class-a/b/c findings + 0 new class-d** | Memory 268-row table + 0 a/b/c + V25-MEM-01 = V24-MEM-01 verbatim carry-over (class-d UPHELD by P25 reviewer as final arbiter) + reviewer APPROVE. DivMMC 132-row table + 0 a/b/c + V24-DIVMMC-01/02 re-verified stable + reviewer APPROVE. NMP 347-row table + 0 a/b/c + 18 prior-fix re-verifications + reviewer APPROVE (4-cycle convergence: P22→P23→P24→P25 all zero-source-delta). CPU 268-row table + 0 a/b/c + 20 prior-fix re-verifications + 7 antipattern re-audits clean + reviewer APPROVE. Differential P24→P25 EMPTY for ALL subsystems. **All 4 subsystems convergence stable across 3 verification windows.** Tests: ctest 38/38, FUSE 1356/1356, regression 33/0/0. |
| **Total (25 passes)** | | **109 class-(a) + 23 class-(b) + 47 class-(c) + 3 follow-ups; ALL 4 SUBSYSTEMS CONVERGENCE-STABLE ACROSS 3 WINDOWS** | **TASK 2 BOOT-CRITICAL SUBSYSTEM AUDIT FULLY CONCLUDED.** Memory (P14+P24+P25), DivMMC (P21+P24+P25), NMP (P22+P24+P25), CPU (P23+P24+P25). **10 class-(d) architectural items pending user authorization** (8 prior + V24-MEM-01 + V24-DIVMMC-02). No further iteration warranted absent new upstream VHDL or new emulator code touching scope. |
