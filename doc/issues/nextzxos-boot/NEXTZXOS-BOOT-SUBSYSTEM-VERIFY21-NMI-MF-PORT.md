# NEXTZXOS Boot Subsystem — Verify-Pass 21 — NMI + Multiface + Port + NextREG

**Branch:** `task2/verify21-nmi-mf-port`
**Integration HEAD (pre-audit):** `94d41ed`
**Audit HEAD (post-fixes, pre-report):** see git log of this branch
**Date:** 2026-05-11
...
were complete (per Pass-21 prompt).

## Findings summary

| ID | Class | Surface | Description |
|---|---|---|---|
| V21-NMP-01 | (c) | NR 0x03 read | bit 7 `nr_palette_sub_idx` hard-wired 0 — wired through `PaletteManager::nine_bit_first_written()` |
| V21-NMP-02 | (c) | Port handler | CTC mask `0xF8FF` aliased 0x1C3B..0x1F3B to channels 0..3; tightened to `0xFCFF` |
| V21-NMP-03 | (c) | NR 0x07 read | `act` field ignored `expbus_eff_en` gate; wired to surface `0` when expbus active |

3 findings — all class-(c) inert divergences. No class-(a)/(b) found
this pass.
...
| nextreg_integration | 262/262 PASS | 272/272 PASS (+10 new V21 rows) |
| nextreg unit | 21/21 PASS | 21/21 PASS |
| port_test | 100/101 PASS, 1 expected-FAIL stub | 101/101 PASS (NR85-03b expectation flipped) |
...
| A30 | 0xF8FF→0xFCFF | port 0x183B..0x1B3B CTC | RW | zxnext.vhd:2690, ctc.vhd:128-137 | ✗ V21-NMP-02 | A10=1 was don't-care; aliased 0x1C3B-0x1F3B to ch 0..3 |
| A31 | 0x00FF/0x006B | port 0x6B DMA ZXN | RW | zxnext.vhd:2643 | ✓ | NR 0x82 b5 (port_dma_6b_io_en) |
| A32 | 0x00FF/0x000B | port 0x0B DMA z80 | RW | zxnext.vhd:2643 | ✓ | NR 0x85 b1 (port_dma_0b_io_en) |
| A33 | 0x00FF/0x00E7 | port 0xE7 SPI CS | W | zxnext.vhd:2620 | ✓ | NR 0x83 b3 (port_spi_io_en) |
| A34 | 0x00FF/0x00EB | port 0xEB SPI data | RW | zxnext.vhd:2621 | ✓ | NR 0x83 b3 |
| A35 | 0xFFFF/0x103B | I2C SCL | RW | zxnext.vhd:2630 | ✓ | NR 0x83 b2 |
| A36 | 0xFFFF/0x113B | I2C SDA | RW | zxnext.vhd:2631 | ✓ | NR 0x83 b2 |
| A37 | 0xFFFF/0x133B | UART Tx | RW | zxnext.vhd:2639 | ✓ | A15:11=00010, XOR(A10,A9∧A8)=1; NR 0x83 b4 |
| A38 | 0xFFFF/0x143B | UART Rx | RW | zxnext.vhd:2639 | ✓ | NR 0x83 b4 |
| A39 | 0xFFFF/0x153B | UART Sel | RW | zxnext.vhd:2639 | ✓ | NR 0x83 b4 |
| A40 | 0xFFFF/0x163B | UART Frame | RW | zxnext.vhd:2639 | ✓ | NR 0x83 b4 |
| A41 | 0x00FF/0x00E3 | DivMMC ctrl | RW | zxnext.vhd:2608 | ✓ | NR 0x83 b0 |
| A42 | 0x00FF/0x001F | Kempston1 | R | zxnext.vhd:2674 | ✓ | NR 0x82 b6 + port_1f_hw_en (G129) |
| A43 | 0x00FF/0x0037 | Kempston2 | R | zxnext.vhd:2675 | ✓ | NR 0x82 b7 + port_37_hw_en |
| A44 | 0x0FFF/0x0ADF | Mouse buttons | R | zxnext.vhd:2668 | ✓ | A11:8=A (V18-NMP-01); NR 0x83 b5 |
| A45 | 0x0FFF/0x0BDF | Mouse X | R | zxnext.vhd:2669 | ✓ | A11:8=B; NR 0x83 b5 |
| A46 | 0x0FFF/0x0FDF | Mouse Y | R | zxnext.vhd:2670 | ✓ | A11:8=F; NR 0x83 b5 |
| A47 | 0xFFFF/0xBF3B | ULA+ ctrl | RW | zxnext.vhd:2685 | ✓ | NR 0x85 b0 |
| A48 | 0xFFFF/0xFF3B | ULA+ data | RW | zxnext.vhd:2686 | ✓ | NR 0x85 b0 + ulap_mode "01" gate |
| A49 | 0xFFFF/<magic> | Magic-port debug | W | (jnext-only) | ✓ | host-side debug aid; no VHDL counterpart |
| A50 | 0x00FF/0x003F | MF+3 readback | R | zxnext.vhd:4310-4316, 2612 | ✓ | mode_p3 + invisible_eff=0 |
| A51 | 0x00FF/0x00BF | MF128 var-A readback | R | zxnext.vhd:4319, 2612 | ✓ | mf_type=01 + invisible_eff=0 |
| A52 | 0x00FF/0x009F | MF128 var-B readback | R | zxnext.vhd:4319, 2612 | ✓ | mf_type=10 + invisible_eff=0 |
...
| B003 | 0x02 | handler | handler | :5117-5119, :3830-3872 | :5891 | ✓ | sw NMI strobes + bus_reset b7 + iotrap b4 + reset_type FSM; Pass-3 fixed bus_reset, V14 NR 0x28 sub-idx alignment |
| B004 | 0x03 | handler | handler | :5121-5151 | :5894 | ✗ V21-NMP-01 | bit 7 was 0 — now palette_sub_idx |
...
| B008 | 0x07 | handler | handler | :5789-5791 | :5902 | ✗ V21-NMP-03 | act field needs expbus_eff_en gate |
...
| B048 | 0x35..0x39 | handlers (=0) | =0 | :4857-4875 | none (others=>'0') | ✓ | sprite attr no-inc; WO (G149) |
| B049 | 0x40 | handler | =palette_idx | :5374-5376 | :6036 | ✓ | palette idx; resets sub_idx |
| B050 | 0x41 | handler | =palette.read_9bit-ish | :5378-5382 | :6039 | ✓ | palette dat(8:1); auto-inc idx |
...
| B057 | 0x50..0x57 | handler | handler | :4880 | :6060-6081 | ✓ | MMU banks |
| B058 | 0x60 | handler (=0) | =0 | :4883-4885 | none | ✓ | copper data + addr++; WO (G149) |
| B059 | 0x61 | handler | =copper.read | :5426-5427 | :6084 | ✓ | copper addr LSB |
| B060 | 0x62 | handler | =copper.read | :5429-5431 | :6087 | ✓ | copper mode + addr hi |
| B061 | 0x63 | handler (=0) | =0 | :5433-5437 | none | ✓ | copper data + addr++ alt; WO (V15-NMP-01) |
| B062 | 0x64 | handler | =copper.read | :5441-5442 | :6090 | ✓ | copper offset |
...
| B072 | 0x75..0x79 | handlers (=0) | =0 | :4861-4875 + :4916 | none | ✓ | sprite attr w/inc; WO |
...
| B075 | 0x81 | handler | (0x80 | & 0x78) | :5491-5496 | :6126 | ✓ | bit 7 = i_BUS_ROMCS_n idle 1; bit 2 + speed bits literal 0 (Pass fix) |
...
| B108 | 0xC2 | RO+latch | (cached) | :4894 + :2050-2085 | :6233 | ✓ | NMI return LSB latched at NMIACK (G88) |
| B109 | 0xC3 | RO+latch | (cached) | :4895 + :2050-2085 | :6236 | ✓ | NMI return MSB latched at NMIACK |
...
| G28 | NextReg.regs_ (256 bytes) | save_state | ✓ |
...
- **NR mux bit-position composition** ("literal '0' / '1'" lines that
  need read masking): NR 0x68 b1 (V20-NMP-02), NR 0x07 (V21-NMP-03 this
  pass), NR 0x03 b7 (V21-NMP-01 this pass) — full re-sweep of every NR
  read mux entry against the table; remaining literal-zero positions
  (NR 0x0B b6/b3:1, NR 0x6A b7:6, NR 0xC6 b7/3, NR 0xCE b7/3, NR 0x4C
  b7:4, NR 0x71 b7:1, NR 0x2F b7:2, NR 0xC4 b6:2, NR 0xCC b6:2, NR 0xA0
  b7:6/b2:1, NR 0xA2 b5, NR 0xA8 b7:1, NR 0xC8 b7:2, NR 0x9B b7:4, NR
  0xC0 b4, NR 0xD8 b7:1, NR 0xDA b7:2, NR 0xF8 b7) all confirmed
  correctly masked or composed.

## Convergence trajectory

| Pass | Findings | Class breakdown |
|---|---|---|
| 11 | 8 | mixed |
| 12 | 17 | (a)+(b)+(c) |
| 13 | 7 | mostly (c) |
...
| 17 | 8 | (b)+(c) |
| 18 | (NMI/MF-Port) ~4 | (c) |
| 19 | (NMI/MF-Port) ~4 | (c)+NIT |
| 20 | (NMI/MF-Port) 1 (V20-NMP-02) | (c) |
| 21 | **3** (V21-NMP-01, 02, 03) | **all (c)** |

The NMI/MF/Port subsystem is **approaching convergence**: only 3 findings
this pass, all of them inert (class-(c) readback divergences with no
boot-path consumer at present). No class-(a)/(b) was found.

## Test enumeration

- nextreg_integration_test: V21-NMP-01-A..F (6 rows) + V21-NMP-03-A/B/C-pre/C-post (4 rows)
- port_test: V21-NMP-02-A/B (2 rows) + NR85-03b expectation flipped (1 row updated)

Total new test rows: **12** (10 nextreg + 2 port). Plus 1 existing row
flipped from expected-FAIL to expected-PASS.
