...
4. **~~Scope of DMA delay into dma.cpp~~** — ~~RESOLVED: per-frame granularity (option A)~~ **SUPERSEDED 2026-07-26 (GH #102 session 4): per-INSTRUCTION.** The predicted divergence arrived — TX-1696 rebooted via a stale-delay wild-jump — so `dma_.set_dma_delay(im2_.dma_delay())` moved to the top of `Emulator::step_one_instruction()` (zxnext.vhd:2001-2010 clocks it per CPU cycle; per-instruction is jnext's finest granularity). Historical per-frame analysis preserved below.
