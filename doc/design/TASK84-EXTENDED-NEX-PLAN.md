...
## 9. Hand-asserted test summary (relocated from TRACEABILITY-MATRIX.md, GH #196 phase 1.5)

This section relocates the Extended/self-streaming NEX (`test/core/extended_nex_test.cpp`)
...
citation, and its `Test ID` column named ranges (`XNEX-01..04`) rather than literal IDs —
nothing in it was ever computed by `test/refresh-traceability-matrix.pl`, so a table
living inside a mostly-generated file read as generated when it was not (GH #192). It
belongs beside this design doc, the one that owns the feature it documents, not wherever
it happened to get filed when commit `7f6d0afd` landed extended-NEX streaming and
stackless NMI side by side.
...
- **XNEX-01..04** (pass) — Generated NEX parsing, exact payload boundary, and closed-file form. Reference: NEX header offset 140; GH #29.
- **XNEX-05..08** (pass) — Register handle, seek/read, position, and file metadata. Reference: NextZXOS `F_SEEK`, `F_READ`, `F_FGETPOS`, `F_FSTAT`.
- **XNEX-09..13** (pass) — Read-only same-directory companion sandbox. Reference: host bridge security contract; GH #84 `ATICATAC.CFG`.
- **XNEX-14..16** (pass) — File map and port `$EB` stream start/end. Reference: NextZXOS `DISK_FILEMAP`, `DISK_STRMSTART`, `DISK_STRMEND`.
- **XNEX-17** (pass) — Memory-address file-handle delivery. Reference: NEX header offset 140 values `$4000..$FFFF`.
- **XNEX-18..20** (pass) — Sector framing, interleaved-handle isolation, final-sector padding, and file-map refill. Reference: NextZXOS streaming contract.
- **XNEX-21..23** (pass) — Initialized direct-load SDHC state and synthetic raw CMD18 overlay. Reference: SD SPI CMD17/CMD18; GH #84.
- **XNEX-24** (pass) — Post-init GUI NEX load activates the otherwise-dormant host bridge. Reference: JNext File → Open lifecycle.
- **XNEX-25..27** (pass) — GUI/CLI soft and hard resets disarm host calls, streaming state, and synthetic SD overlays. Reference: direct-NEX lifecycle isolation; GH #29/#84.
