...
- **Line 315** — `clip_x_shift` logic was: `(mode_80col_ && render_width==640) ? 1 : 0`. **CORRECTED 2026-05-02 (Strategy B):** `clip_x_shift = 1` in BOTH col-modes. Per VHDL `tilemap.vhd:424` the clip comparator runs against `hcounter_i` (9-bit, 0..319 visible, 7 MHz) — NOT against `hcount_effsub`. So the clip decision is paced at hcounter_i / 320-grid regardless of col-mode. In jnext's 14 MHz / 640-cell output, each pair of cells `(2k, 2k+1)` shares one `hcounter_i` value; mapping `screen_x → hcounter_i` is `screen_x >> 1` in both modes. The col-mode (`tilemap.vhd:228`) only affects which SOURCE PIXEL each output cell reads (14 MHz `hcount(10:1)` in 80-col vs 7 MHz `hcount(10:2)` in 40-col); it does NOT affect the clip-comparator rate. Original plan text saying `mode_80col_ ? 1 : 0` was wrong — conflated source-pixel-index rate with clip-comparator rate. Implication: under default clip `[clip_x1=0, clip_x2=0x9F]` both col-modes paint all 640 cells; one `clip_x` register unit covers 4 output cells. TM-112-style assertions land at `dst[0x40..0x43]` regardless of col-mode.
