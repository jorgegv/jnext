...
| ~~**NR 0x43 bits 1-3**~~ + **NR 0x6B bit 4** active palette select | `Tilemap::render_scanline` → `PaletteManager::tilemap_colour` | **NR 0x43 b1/b2/b3 DONE** (b1 with the original G10 log, b2/b3 2026-07-30 GH #163 — see the covered table above). **NR 0x6B b4 is still OPEN and has the identical shape**: `Ula::get_active_tilemap_palette()` is replayed per line but has no production consumer, so `tilemap_colour(idx)` still reads the live end-of-frame `active_tm_second_` and a mid-frame tilemap-palette flip collapses to last-write-wins. Fix = the GH #163 change applied to the one tilemap lookup. | TBD (no demo yet) | S |
