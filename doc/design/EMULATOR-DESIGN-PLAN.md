...
### 3.1 VHDL → C++ subsystem mapping

The authoritative hardware spec is the VHDL source at
`cores/zxnext/src/` in the TBBlue/SpectrumNext repository. The table
below maps every VHDL module to its C++ home in jnext and its scope
decision. "Scope" = whether jnext intends to emulate the module. Modules
...
| `video/lores.vhd`                               | yes     | `src/video/lores.*` + `renderer.*`    | 128x96 8-bit + Radastan 4-bit. NOT a layer: substitutes the ULA-slot pixel (zxnext.vhd:6980). NR 0x15 b7 / 0x32 / 0x33 / 0x6A wired + per-line replay. GH #63; plan: [LORES-TEST-PLAN-DESIGN.md](../testing/LORES-TEST-PLAN-DESIGN.md) |
...
- [x] DivMMC SD card `.img` mounting — SdCardDevice SPI backend with CMD0/CMD8/CMD17/CMD24/CMD55+ACMD41/CMD58; SDHC byte addressing; `--sdcard` CLI option (mandatory after Wave 0.3, 2026-05-04). DivMMC ROM is extracted from `/MACHINES/NEXT/enNxtmmc.rom` on the SD image.
...
- [x] Machine-type selection: 48K / 128K / +3 / Pentagon / Next via `--machine` CLI + Qt menu; ROMs extracted from the `--sdcard` image at `/MACHINES/NEXT/{48,128,plus3}.rom` (Wave 0.3, 2026-05-04 — Pentagon substitutes 128.rom per TBBlue convention); fixed port 0x7FFD decode mask; +3 port 0x1FFD paging with 4-ROM selection
...
- [x] Ensure all of David Crespo's tests work the same as Zesarux (verify each one with Zesarux)
  ...
  - [x] 04-dapr-tilemap - verified and fixed - split in 3 tests
...
- [x] **Reduce SKIPs — implement missing features** — per-subsystem plans + per-feature branches. Live counts in `test/SUBSYSTEM-TESTS-STATUS.md`. Per-plan detail in `doc/design/TASK*-PLAN.md` and per-session detail in `.prompts/*.md`.
  ...
  - [x] Sprites
  - [x] Layer 2
  - [x] Compositor (incl. UDIS-01/02 + UDIS-03 `ula_blend_mode`)
  - [x] Port Dispatch
  - [x] NextREG (bare + integration)
  - [x] Memory/MMU (incl. shadow-screen routing)
  - [x] Copper
  - [x] DivMMC+SPI
  - [x] UART+I2C/RTC
  - [x] Input (incl. NR 0x0B pin-7 re-audit)
  - [x] CTC+Interrupts
  - [x] ULA Video
  - [x] Audio
...
- [x] General optimization plan — DONE (Task 65 closed 2026-07-17: architecture assessed, perf optimized ~2.8×; refactor + magic-numbers surveyed → WONT; copper/beast 400% tail → WONT)
  ...
  - [-] General code refactor and tidy up (/simplify) - Probably subsystem by subsystem, then globallly — **WONT (Task 65, 2026-07-17)**: a 5-subsystem parallel survey found the codebase already mature/disciplined/VHDL-faithful — zero leftover commented-out code, no dead paths, no obsolete logic, minimal accidental cruft; every subsystem "not worth it". Only actioned: deleted 2 confirmed dead functions (`PaletteManager::rebuild_argb`, `Beeper::current_level`). Remaining candidates are save-state-coupled, risk-adjacent, or VHDL-faithful micro-duplication not worth the traceability loss.
  - [-] Replacement of magic number with named constants where possible — **WONT (Task 65, 2026-07-17)**: a 4-subsystem parallel survey found only ~18 genuine bare literals tree-wide (~6 worthwhile), every subsystem "not worth it" — the code is already densely constanted and VHDL-commented, so a dedicated pass is churn/regression risk for negligible gain. (A handful of optional spot-fixes noted, e.g. `contention.cpp` `320`/`456` duplicating the LUT dims.)
  ...
  - **Note (RESOLVED):** the 75 FPS @ 400% bottleneck is fixed — boot-nextzxos now runs ~284 FPS @ 400% (161M T/s), well past the 200 FPS target.
  - [-] copper-demo / beast own-content 400% tail — **WONT (Task 65, 2026-07-17)**: live re-measure copper-demo 163.5 fps (target 200, 1.22× short) / beast 178 fps (target 240, 1.35× short); still miss because `Copper::execute`'s own WAIT/MOVE work (8%/12%) is irreducible and the only gap-closing options are months-long, high-risk core changes with uncertain or *negative* payoff (internal cycle-accurate core is ~4× slower; JIT rejected). Negligible user impact (both run flawlessly at 100%; only fast-forward on the two heaviest demos falls short, still ~3×). Full evaluation in [TASK27-P2-PROFILE-COPPER-BEAST.md](TASK27-P2-PROFILE-COPPER-BEAST.md) §8.
...
- [x] Evaluate other "shortcut" options ✓ (2026-05-17: `--bypass-tbblue-fw` built as the shortcut; 2026-07-10: superseded as a boot path by the working native boot; 2026-07-11: the flag and its C++ route were REMOVED per user decision — native firmware-faithful boot is the only path)
...
- [ ] Redefinable debugger keys (or perhaps named key combinations? e.g. "borland", "cspect", "zesarux",...) — [issue #1](https://github.com/jorgegv/jnext/issues/1)
- [ ] Save screenshots in .SCR format — [issue #18](https://github.com/jorgegv/jnext/issues/18)
- [ ] Save screenshots with automatically generated name (without asking the user for the name - fast!) — [issue #19](https://github.com/jorgegv/jnext/issues/19)
- [-] Allow specifying automatic screenshot time in T-states — **WONT (Task 65, 2026-07-17, low-value scope call)**: seconds (`--delayed-screenshot-time`) and frames (`--delayed-screenshot-frames` / `--delayed-automatic-exit-frames`) are done and cover every real need. jnext renders per-frame (the framebuffer is only complete at a frame boundary), so a screenshot triggered at an arbitrary mid-frame T-state would still capture the last *completed* frame — sub-frame T-state precision has no meaning without per-scanline render capture. Not worth the CLI surface for zero observable gain.
- [-] Layer 2 G9-06 weak-case documentation comment (`hc_eff <= hc + 1`, VHDL-internal pipeline signal) — **WONT (Task 65, 2026-07-17)**: gated on "a cycle-accurate refactor" (see the internal-core / NR-write-priority WONTs), which the copper/beast 400% assessment ruled out as negative-payoff. This sub-pixel per-column pipeline signal is unobservable under jnext's per-scanline accuracy model, so it stays a documented modeling limitation rather than an observable per-column assertion. (The two category-F skips formerly tracked here — Copper ARB-06, Port Dispatch NR82-02 — are real `check()` rows now.)
- [x] Traceability matrix — close the VHDL-citation gap. `test/refresh-traceability-matrix.pl` now fills the `VHDL file:line` column from four **row-local** evidence tiers (the row's own `check()`/`skip()` call, a comment block naming the ID, the first call after a table-driven ID literal, the plan doc's row) and validates every citation against the real FPGA source tree. **1608 → 579 uncited rows** (1029 filled; 30 of the residual are Z80N, `missing` by design). Per-suite tombstones for the two suites with no VHDL counterpart at all (Rewind → `(jnext-internal)`, SD Card → `(SD SPI spec)`). Deliberately rejected: banner-comment and nearest-unrelated-comment tiers — both prototyped, both attribute a neighbouring row's VHDL lines to this one, and a plausible-but-wrong citation is worse than an honest `—`. The table-driven tier is restricted to rows with no call of their own for the same reason (an unrestricted version shipped 5 wrong citations and was caught in review). Extractor pinned by `make traceability-selftest`, whose fixture rows include the refusals (the row count is the target's own pinned `Total:`, deliberately not restated here — it has gone stale three times). Hand-written citations are never overwritten; disagreements are reported instead. The remaining `—` rows are a real, visible gap — the fix is to cite the VHDL in the test source and re-run.
  - [ ] Row-level citation tombstones (`(round-trip)`, `(register declaration)`) — NOT implemented. Only the two per-suite tombstones landed. Rows that genuinely have nothing to cite (pure getter/setter round-trips, register-declaration readbacks) are currently indistinguishable from rows whose citation is merely missing: both read `—`. Classifying them needs a per-row judgement no extractor can make from the source, so it wants either an explicit marker in the test itself or a hand-curated list.
- [ ] Specific test capture and playback workflow — [issue #20](https://github.com/jorgegv/jnext/issues/20) (needs deep evaluation first: may be subsumed by the Scriptable Debugger):
...
- [ ] Allow selecting and copying assembly code from the ASM view to paste into editors and such (try to make the copy only include the ASM code, and not the addr, the opcode bytes, etc.) — [issue #21](https://github.com/jorgegv/jnext/issues/21)
...
Complex ones:
- [x] DEVELOPMENT documentation and process — [issue #44](https://github.com/jorgegv/jnext/issues/44). **DONE**: the developer guide, 38 pages in six chapters, source under `src/doc/developer-guide` and a committed mkdocs render under `doc/developer-guide` (`make docs-devguide` / `read-devguide`), staleness-gated by `docs-devguide-check` as the third part of `docs-check`. Figures are **Graphviz** `.dot` sources rendered to committed SVG, not mermaid: mkdocs-material fetches `mermaid.min.js` from a CDN at page load, which is blank for exactly the offline reader a committed render exists for. Covers architecture, a frame end to end, all ~30 subsystems mapped to their VHDL counterparts, the test system, build/packaging/CI, and the contribution process.
  - Written against the CODE, not against this plan — and that was justified. The guide's authors found this document wrong about the port dispatcher's data structure, the Copper's ports and its location, contention (the pre-computed LUT has no runtime caller), the Z80N opcode count, the RAM default, sprite collision, ayemu, `DebuggerInterface`, and the debugger in about a dozen further ways. **Those corrections were recorded in the guide, not here; this document remains the roadmap and remains unreliable as a description.**
  - Two defects were found by writing it and are tracked separately: [#203](https://github.com/jorgegv/jnext/issues/203) (Step Out never stops — `check_step_out()` has no caller) and [#204](https://github.com/jorgegv/jnext/issues/204) (owner-specific absolute paths hardcoded in scripts and tools).
...
- [ ] Scriptable debugger for accurate T-state, Scanline and Frame events, etc. - See plan at @doc/design/SCRIPTABLE-DEBUGGER.md — [issue #26](https://github.com/jorgegv/jnext/issues/26) (design needs refinement before implementation)
- [ ] Next-specific snapshot save/load format (design from scratch — only NEX exists today; needs own design doc + external/community comments; seed material in the G66 tombstone in [KNOWN-FUNCTIONALITY-GAPS-AND-PLAN.md](../issues/KNOWN-FUNCTIONALITY-GAPS-AND-PLAN.md); was Task 13a) — [issue #27](https://github.com/jorgegv/jnext/issues/27) (extending the NEX format is a likely-preferable option to a clean-sheet design)
- [-] Develop a new Z80N core derived directly from VHDL description, selectable at runtime (`fuse` vs `internal`) — **WONT (Task 65, 2026-07-17)**: all three original justifications have collapsed. (1) *Performance*: negative — INTERNAL-Z80N-CORE-PLAN.md's own analysis puts cycle-accurate microcode stepping ~4× slower than the current instruction-level interpreter, so it cannot help the copper/beast 400% tail (also WONT) and would hurt normal perf. (2) *Accuracy*: marginal — the FUSE core already passes 1356/1356 FUSE Z80 tests and Z80N is fully implemented/verified. (3) *Licensing*: moot — FUSE is GPLv2-**or-later** (`third_party/fuse-z80/`), fully compatible with jnext's GPLv3; the plan's "BSD is cleaner" point only matters for a permissively-licensed project, which jnext is not. Weeks of high-risk work for no net win. Plan retained at [INTERNAL-Z80N-CORE-PLAN.md](INTERNAL-Z80N-CORE-PLAN.md) for reference.
...
- [-] Model cycle-accurate CPU/Copper NR write priority — **WONT (Task 65, 2026-07-17), resolved as option (a)**. VHDL `zxnext.vhd:4769,4775-4777` enforces Copper-wins priority on same-28 MHz-cycle NR writes with CPU deferred. JNEXT serializes CPU and Copper NR writes (no shared bus) — priority is implicit in tick-loop order, not emulator-enforced. This item was explicitly gated on "a cycle-accurate timing refactor", which the copper/beast 400% assessment ruled out as negative-payoff — so option (b) (cycle-accurate CPU/Copper scheduling) will not happen. Adopting **option (a)**: leave priority as a test-harness convention, documented here as a known modeling limitation. The ARB-01/02/03 tests satisfy the VHDL outcome by ordering stimulus manually and are accepted as sufficient under the project's per-scanline accuracy model.
- [x] Per-scanline display state replay — DONE (demo-driven; residual read-live and added only when a demo needs it — deliberate, not a coverage gap). Shipped per-line snapshot / change-log+replay classes: palette content (beast sky gradient), L2 X/Y scroll (G10, beast parallax), L2 bank NR 0x12/0x13 (G09), L2 + ULA clip windows, NR 0x15 layer-priority + sprite-enable (G02 — change-log class was DORMANT with zero callers until GH #73 wired it 2026-07-23; rows PSCAN-G02-01..05 in the compositor suite prove mid-frame priority/sprite-en changes, cross-frame persistence and the vblank flush; b7 stays with the LoRes snapshot, b6/b5/b1 stay frame-granularity SpriteEngine state), NR 0x14 transparent RGB (G04), NR 0x68 blend/stencil bits, ULA scroll + palette-select (NR 0x43 / 0x6B b4) + border, fallback colour NR 0x4A, ULA enable, tilemap scroll + NR 0x6C/0x6E/0x6F fetch state (TX-1696 HUD/playfield split), sprite attribute/pattern multiplexing, and the Nirvana-class attribute-write mux (`AttributeMux`, G12, 2026-07-14). Residual still read live: NR 0x4B/0x4C transparency indices, NR 0x6B tilemap mode bits, NR 0x70 L2 resolution mode, port 0xFF Timex mode, sprite/tilemap clip windows. Driven by demo, not by completeness — coverage matrix in [doc/design/PER-SCANLINE-DISPLAY-STATE-AUDIT.md](PER-SCANLINE-DISPLAY-STATE-AUDIT.md) (NOTE: the matrix itself lags the code — several rows it lists as open are in the shipped list above).
...
| Issue | Detail | Status |
|-------|--------|--------|
| ~~**DAC audio output buzzing**~~ | RESOLVED 2026-07-20 — **not an emulator defect** ([issue #38](https://github.com/jorgegv/jnext/issues/38), closed not-planned). The demo drove channel D via port `0xDF`, which is the Specdrum *mono* port and writes channels A **and** D together (`zxnext.vhd:2661,2664`), and had B/D on the wrong sides; both sides therefore carried `sample_a + sample_b`, measured L/R correlation exactly 1.000000. `sample_b` is the 3rd harmonic of `sample_a`, and that sum is the buzz. jnext decodes per `soundrive.vhd:112-113` correctly throughout — which is why ZEsarUX reproduced it identically, and why real Next hardware sounds the same (owner-verified). Fixed in `demo/dac_demo/dac_demo.c`. | Closed |
...
| **Windows large early-startup stack frame** | On the MinGW Windows build, an early code path (before `main`, same `__chkstk` address for `--version`/`--help`/`--headless`) reserves a >2 MB stack frame that overflows MinGW's default 2 MB reserve. Worked around by reserving 16 MB on Windows (`-Wl,--stack` in CMakeLists, WIN32-only; harmless — Linux uses an 8 MB default and never hit it). The underlying large frame was not identified (no plain >256 KB array found via grep; likely an aggregate/inlined frame). Investigate what allocates it and whether it should be heap-backed. Low priority — the reserve bump fully resolves the crash. | Open |
