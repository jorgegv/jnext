...
- [ ] **Reduce SKIPs — implement missing features** to un-skip the plan rows that have VHDL-traced tests but no C++ implementation yet. Work per-subsystem, one branch per feature, with code review and regression. See `test/SUBSYSTEM-TESTS-STATUS.md` for live per-subsystem counts.
  ...
  - [~] DivMMC+SPI
  - [x] UART+I2C/RTC — Task 3 SKIP-reduction plan ([`doc/design/TASK3-UART-I2C-SKIP-REDUCTION-PLAN.md`](TASK3-UART-I2C-SKIP-REDUCTION-PLAN.md)) closed end-to-end 2026-04-24 Phase 0 → 4. Bit-level `UartTxEngine` + `UartRxEngine` (7+8 VHDL states per `uart_tx.vhd` / `uart_rx.vhd`) opt-in via `set_bitlevel_mode()`; RX FIFO widened to `uint16_t` with 9th-bit error flag; `I2cRtc` expanded to full DS1307 (64-byte regs, write-path, 12h mode, CH bit freezes oscillator, pointer wrap at 0x3F, NVRAM 0x08-0x3F, `use_real_time_` switch for deterministic testing). Emulator gets `inject_pi_i2c1` / `inject_joy_uart_rx` + port-enable gate wrap per NR 0x83 bits 2+4. `uart_test.cpp` **106/58/0/48 → 92/92/0/0** (ZERO skips); new companion suite `test/uart/uart_integration_test.cpp` **12/12/0/0** covering INT-01..06 (UART 0/1 RX+TX vectors), GATE-01..03 (port-enable gate), DUAL-05/06 (UART 1 joy-pad mux), I2C-10 (pi-i2c1 bus mux). 4 plan-drifts caught: 12-fail-staleness (fixed via `174fa56`), IM2 vector-numbering (2 vectors not 4), NR 0x82 vs NR 0x83 gate-register bit mapping, RTC-06/07 BCD-fix-unnecessary (already resolved). BAUD-02/03 D-UNOBSERVABLE (prescaler low 14 bits not exposed to any VHDL read path). Full audit in `doc/testing/audits/task3-uart-i2c-phase4.md`.
  - [~] Input — Task 3 SKIP-reduction plan (`doc/design/TASK3-INPUT-SKIP-REDUCTION-PLAN.md`) landed 2026-04-21/22 Phase 0 → 4. 5 NEW classes added in `src/input/` (`Joystick` NR 0x05 decoder + port-1F/37 composer; `KempstonMouse`; `Md6ConnectorX2` single-instance time-multiplexed FSM; `MembraneStick` Sinclair/Cursor/Cursor adapter from `keyjoy_64_6.coe` oracle; `IoMode` NR 0x0B pin-7 mux). Keyboard extended (extended-key matrix + shift hysteresis + cancel_extended_entries). NR 0x06 NMI AND-gate. CTC bug fix (`on_zc_to` callback split from `on_interrupt`). `input_test.cpp` `149/23/0/126` → `139/133/0/6`; new companion suite `test/input/input_integration_test.cpp` 7/5/0/2. 6 + 2 = 8 remaining skips are all F-blocked (UART pin-7 modes blocked on UART+I2C plan; FE-04 MIC^EAR + FE-05 expansion bus). Plan-text inconsistencies caught and queued: §3.7 SINC1/SINC2 keymap swap, EXT-18/19 fold-row labels.
  ...
  - [x] Audio — Task 3 SKIP-reduction plan ([`doc/design/TASK3-AUDIO-SKIP-REDUCTION-PLAN.md`](TASK3-AUDIO-SKIP-REDUCTION-PLAN.md)) closed end-to-end 2026-04-24 Phase 0 → 4, plus ad-hoc Waves E+F for per-PSG isolation + port-dispatch gaps surfaced mid-plan. `audio_test.cpp` **200/127/0/73 → 132/132/0/0** (ZERO skips). New companion suites: `test/audio/audio_nextreg_test.cpp` **25/25/0/0** (NR 0x06/0x08/0x09/0x2C-2E audio handlers + beeper XOR + exc_i gating) and `test/audio/audio_port_dispatch_test.cpp` **16/16/0/0** (Soundrive + Covox variants + AY FFFD/BFFD/BFF5 decode). `input_integration_test` extended with Group FE-READ (BP-04 + BP-20..23 re-homed, 7/5/0/2 → 12/10/0/2). New `src/audio/i2s.{h,cpp}` stub mirroring `audio_mixer.vhd:89-90`. NR 0x06 bit-decode fix (VHDL:6389 aymode_i = psg_mode[0], was `(v & 0x03) == 1`), NR 0x2C/2D/2E new handlers, NR 0x08 bit 3 dac_enabled_ shadow, NR 0x84 bit 5 AND NOT bit 2 mono-AD gate per VHDL:2433. 6 port handlers added (0x005F, 0x003F, 0x00B3 new; 0x00FB rework; 0xBFF5 new; BFFD read on +3 alias). All changes VHDL-cited per VHDL-as-oracle rule. Full audit in `doc/testing/audits/task3-audio-phase4.md`.
