...
//     The struct is naturally 16 B (aligned to 8 B). At 2 M entries the
//     working set is 32 MB virtually; we mmap MAP_ANONYMOUS so the kernel
//     only commits pages actually touched (typical 128K/Next runs touch
//     a few hundred KB of physical RAM).
//   * Allocated only when `--profile` is set; non-profile runs see zero
//     cost in the CPU hot path (single `if (profile_active_)` predicted
//     branch).
//
...
//
// Note on the "physical 8K page" lookup: profile_step() takes the page
...
class Profiler {
public:
    // 8 K pages × 8 K offsets = 2 097 152 entries (16 MB virtual).
