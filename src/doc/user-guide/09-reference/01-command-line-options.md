...
:   Load a program. The format is detected from the extension: `.nex`,
    ...
    *extended* NEX. If its header asks for the file to be kept open
    ...
    header says `file_handle` 0, jnext does what the Next’s own loader
    does: it loads only the declared banks, ignores the trailing bytes
    and starts the program. More than 16 KB of ignored bytes also logs a
    warning.
