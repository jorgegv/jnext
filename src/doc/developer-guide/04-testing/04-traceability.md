...
- **Test ID and description** — the row's own `check("ID", "description", ...)`
  or `skip("ID", "reason")` call. The description's position varies between
...
An edit is overwritten by the next run and fails the gate in the meantime. That
is why the older bookkeeping-drift classes no longer exist: there is no
hand-written side left to disagree with. The one report that survives is a plan
doc and a test source citing *different* VHDL for the same row — signal about
the spec, not about bookkeeping.
...
three `|`-separated fields per record — section, row ID, description. It covers
the one case neither a test source nor a plan doc can answer for, a planned row
of a suite that has *no* plan doc because it has no VHDL counterpart —
`rewind_test` and `sdcard_test`, tombstoned as `(jnext-internal)` and
`(SD SPI spec)`. It is parsed strictly: a malformed record or a duplicate
`(section, id)` is a refusal, not a dropped row. It is not a place to assert
coverage — every row in it emits as `missing`, because nothing runs it.

## What the generator refuses to do

Its exit codes are a vocabulary, not decoration: `0` clean, `1` rewritten but
still under-recording, `2` refusal with nothing written, `3` internal error.

- **It refuses when a declared suite is unaccounted for.** Every suite in
  `test/unit-tests.conf` must be either *traced* — mapped to a `##` section, the
  one genuinely editorial fact left — or *tombstoned* with a written reason.
  That replaced an advisory warning which had saturated: the traced-suite count
  sat at 28 for a whole release series while the manifest grew 49 → 80, each
  addition arriving as one more name on a warning line that already listed
...
- **It refuses to invent a row.** A row the tests assert but the matrix does not
  ...
  recording nothing, and would be self-consistent forever after.
- **It refuses to guess a source path.** Suite sources are read from CMake, not
  ...
  not, and the two ESP-01 suites live under `src/esp01/test/`. A convention
  right 92% of the time is the worst kind.
- **It leaves hand-written citations alone**, reporting disagreements rather
  than overwriting them, and leaves rows marked `<!-- protected -->` untouched.
