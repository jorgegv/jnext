...
Two workflows run by themselves. `ci.yml` tests, and runs on every push to
`main` and every pull request. `release.yml` builds and publishes packages, and
runs on a `v*` tag push.

The rest are dispatched by hand, from Actions → *Run workflow*. They exist
because a platform whose build only runs at release time is a platform whose
breakage is found at release time. `macos-build.yml` builds, packages and
verifies the macOS `.dmg`; `windows-build.yml` produces the three Windows zips,
which is also how a fix build reaches a reporter before the next release; and
`flatpak-build.yml` builds the Flatpak bundle and checks its sandbox
permissions.
...
the need is imaginary. There is exactly one declared exception, the Flatpak
build in `flatpak-build.yml`, and it is written down both in that file's header
and in `doc/RELEASE-PROTOCOL.md` so that it stays a decision rather than
becoming drift. Its permission check is *not* part of the exception: that step
runs `make verify-flatpak-permissions`, the same target a local
...
`windows` (three zips from one Fedora container), `flatpak` (one line, calling
`flatpak-build.yml`), and `macos`.
