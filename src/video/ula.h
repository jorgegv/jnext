...
public:
    ...
    void reset() {
        ...
        // Per-scanline scroll change-log (G08) — clear at reset; the next
        // start_frame_scroll() snapshot will re-baseline from the live
        // (post-reset) values. Kept in sync with Layer2::reset() shape.
...
private:
    ...
    // VHDL zxnext.vhd:5304 (NR 0x26), :5307 (NR 0x27), :5449 (NR 0x68 b2).
    // Each entry captures a snapshot of all three live scroll fields after
    // each setter call, tagged with current_scroll_line_. Two registers
    // (NR 0x26/0x27) plus the fine-scroll bit are stored together in a
    // single struct so a single-write to either register always preserves
    // the unrelated coordinate (avoids "last write wins" interleaving when
    // a Copper alternates between the two on consecutive scanlines).
    struct ScrollChange {
        uint16_t line;             ///< 0..lines_per_frame-1
        uint8_t  scroll_x_coarse;  ///< NR 0x26 snapshot
        uint8_t  scroll_y;         ///< NR 0x27 snapshot
        uint8_t  fine_scroll_x;    ///< NR 0x68 b2 (0 / 1)
    };
    ...
    /// Append a snapshot of (ula_scroll_x_coarse_, ula_scroll_y_,
    /// ula_fine_scroll_x_) to the change log, tagged with
    /// current_scroll_line_. Called from the three scroll setters above.
