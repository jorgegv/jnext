                               ...
                               Mmu& mmu)
{
    ...
    for (int col = 0; col < 32; ++col) {
        ...
        // Flash: swap ink/paper on the active phase. VHDL zxula.vhd:470 —
        // `attr_active(7) and flash_cnt(4) and (not i_ulanext_en) and
        // not i_ulap_en`: ULA+ (and ULAnext) suppresses the flash XOR term,
        // so attr(7) is reinterpreted as a palette-group bit rather than
        // the flash toggle. Wave C (2026-04-23) gates the swap on
        // `!ulap_en_` to unblock row S4.06. (Wave B owns the ULAnext side.)
        if (flash && flash_phase_ && !ulap_en_) {
