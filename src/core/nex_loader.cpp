...
bool NexLoader::load(const std::string& path)
{
    ...
    // host file; when keep_open, Emulator's extended-NEX bridge serves it lazily.
    ...
    if (file_size > expected_size && keep_open) {
        Log::emulator()->info(
            ...
            path, file_size - expected_size);
    } else if (file_size > expected_size && !ignored_payload) {
        Log::emulator()->info(
...
bool NexLoader::apply(Emulator& emu) const
{
    ...
    //    0x18 with (palette.cpp kDefaultUlaRrrgggbb[8], bright black), and
