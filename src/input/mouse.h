...
/// interpretation is host-side (plan row MOUSE-10). Kempston wheel
...
/// applied here — neither has an in-core VHDL consumer (their only two
/// readers are the NR 0x0A read-back at zxnext.vhd:5912 and the
/// o_MOUSE_CONTROL output pin at :1599); both are host-adapter concerns.
/// That absence is asserted, not assumed: plan rows MOUSE-09 / MOUSE-11
/// in test/input/input_test.cpp fail if either is applied here.
///
...
public:
    ...
    /// (plan row MOUSE-09 — VHDL has no in-core consumer).
