...
// friends). Call once, right before the first potentially-slow phase — SD-card
// provisioning, whose confirm prompt / download can take minutes; everything
// after it is the app session: past that point an exit is not a quick CLI
// result, and the injected Enter could submit a command the user typed into
// the waiting shell meanwhile.
