...
unit-test: unit-test-build
	...
	rm -f $$SUMMARY; \
	TESTS="fuse_z80_test z80n_test rewind_test copper_test copper_integration_test mmu_test nextreg_test \
	       nextreg_integration_test input_test input_integration_test ctc_test ctc_interrupts_test layer2_test \
	       uart_test uart_integration_test divmmc_test sdcard_test sprites_test compositor_test compositor_integration_test ula_test ula_integration_test \
	       floating_bus_test videotiming_test contention_test port_test audio_test audio_nextreg_test audio_port_dispatch_test dma_test tilemap_test nmi_test nmi_integration_test"; \
	for t in $$TESTS; do \
		bin="$$BUILD/test/$$t"; \
