...
# The DEVELOPER guide (GH #44) is a second mkdocs site with its own config,
# built and staleness-checked exactly like the user guide above. It differs in
# one way: its figures are GENERATED too. The diagram sources are Graphviz
# `.dot` files and `make docs-devguide` renders them to committed SVG before
# mkdocs runs, so the fingerprint has to cover `dot` as well — graphviz emits
# byte-different SVG across versions for identical input, which would read as
# staleness on a host with a different graphviz, exactly as pandoc did in CI.
#
# Graphviz was chosen over Mermaid deliberately: mkdocs-material's Mermaid
# integration loads mermaid.min.js from unpkg.com at page load, and this guide
# is committed precisely so it can be read offline from a clone. Self-hosting
# it means vendoring a 3.5 MB minified blob (twice — source and rendered
...
DEVGUIDE_OUT      := doc/developer-guide
DEVGUIDE_DOT      := $(wildcard src/doc/developer-guide/diagrams/*.dot)
DEVGUIDE_SVG      := $(patsubst src/doc/developer-guide/diagrams/%.dot,src/doc/developer-guide/img/%.svg,$(DEVGUIDE_DOT))
...
kloc-count:
	@# The subsystem list is DISCOVERED, not written down: every top-level
	@# directory under src/ that holds sources, plus test/. Adding a subsystem
	@# needs no edit here and — the point — cannot silently go uncounted. The
	@# old hard-coded list had already lost src/profiler, and its `if [ -d ]`
	@# guard made an unlisted directory and a deleted one look identical: both
	@# printed nothing. A directory with no .cpp/.h (src/doc, src/save) is
	@# skipped by the same rule that finds the others, so there is no list of
	@# exceptions to maintain either.
	@#
	@# A module shipping its own suite (src/esp01/test — GH #25) counts that
	@# suite under the module, not under test/: the proof ships with the code.
	@#
	@# `grep -h` is load-bearing. With several files grep prefixes every output
	@# line with `filename:`, anchoring the following ^-patterns against the
	@# path instead of the code — so the three comment filters matched NOTHING
	@# and this target counted comments for as long as it existed, while its
	@# own header claimed otherwise. Reported total 165,897 -> 105,803.
...
docs-check:
	@# Runs EVERY part and aggregates, so one stale document never hides the
	@# other. Each part is a self-contained target with its own skip/fail and
...
# Render the developer guide's Graphviz diagrams to committed SVG (needs graphviz)
docs-devguide-diagrams:
	@if ! command -v dot >/dev/null 2>&1; then \
	   printf "$(BADGE_FAIL) FAIL $(RESET) graphviz (dot) not found. It is a\n"; \
	   printf "        documentation-only dependency: install it from your\n"; \
	   printf "        distribution (Fedora: sudo dnf install graphviz). The\n"; \
	   printf "        rendered SVGs are COMMITTED under $(DEVGUIDE_SRC)/img — you only\n"; \
	   printf "        need dot to re-render after editing a .dot source.\n"; exit 1; \
	 fi
	@# Deliberately regenerate ALL of them, not just the ones whose .dot is
	@# newer: the outputs are committed and byte-diffed, and a checkout gives
	@# every file the same mtime, so a timestamp rule would silently skip.
	@for d in $(DEVGUIDE_DOT); do \
	   out=$(DEVGUIDE_SRC)/img/$$(basename $$d .dot).svg; \
	   dot -Tsvg "$$d" -o "$$out" || exit 1; \
	 done
	@printf "$(BADGE_PASS) OK $(RESET) $(words $(DEVGUIDE_DOT)) diagram(s) rendered to $(DEVGUIDE_SRC)/img\n"

# Render the developer guide from src/doc/developer-guide into doc/developer-guide
docs-devguide: docs-devguide-diagrams
	@if ! command -v mkdocs >/dev/null 2>&1; then \
	   printf "$(BADGE_FAIL) FAIL $(RESET) mkdocs not found. It is a documentation-only\n"; \
	   printf "        dependency: install mkdocs-material from your distribution\n"; \
	   printf "        (Fedora: sudo dnf install mkdocs-material; see BUILD.md). The\n"; \
	   printf "        rendered guide is COMMITTED under $(DEVGUIDE_OUT) — open\n"; \
	   printf "        $(DEVGUIDE_OUT)/index.html or run 'make read-devguide'. You\n"; \
	   printf "        only need mkdocs to re-render after editing $(DEVGUIDE_SRC).\n"; exit 1; \
	 fi
	$(GUIDE_BUILD_ENV) mkdocs build --strict --config-file $(DEVGUIDE_CONFIG)
	@$(DEVGUIDE_FINGERPRINT) > $(DEVGUIDE_RENDERER)
	@printf "$(BADGE_PASS) OK $(RESET) developer guide rendered to $(DEVGUIDE_OUT)\n"
	@printf "        it is committed: commit the regenerated files alongside your change\n"

# Fail if the committed developer guide (or its SVGs) is stale vs its sources
docs-devguide-check:
	@# Two outputs, one gate: the SVGs under $(DEVGUIDE_SRC)/img are generated
	@# from the .dot sources and committed, and the rendered site under
	@# $(DEVGUIDE_OUT) is generated from the markdown AND those SVGs. Checking
	...
	@if ! command -v mkdocs >/dev/null 2>&1 || ! command -v dot >/dev/null 2>&1; then \
	 ...
	 rc=0; \
	 for d in $(DEVGUIDE_DOT); do \
	   dot -Tsvg "$$d" -o "$$tmp/$$(basename $$d .dot).svg" || rc=1; \
	 done; \
	 ...
	 if [ $$rc -eq 0 ] && [ "$$here" != "$$there" ]; then \
	   ...
	   for d in $(DEVGUIDE_DOT); do \
	     ...
	     cmp -s "$$tmp/$$b" "$(DEVGUIDE_SRC)/img/$$b" \
	       || { printf "$(BADGE_FAIL) FAIL $(RESET) $(DEVGUIDE_SRC)/img/$$b is stale vs $$d\n"; rc=1; }; \
	   done; \
...
read-devguide:
	@if [ ! -f $(DEVGUIDE_OUT)/index.html ]; then \
	   printf "$(BADGE_FAIL) FAIL $(RESET) $(DEVGUIDE_OUT) is not rendered yet.\n"; \
	   printf "        Run 'make docs-devguide' first.\n"; exit 1; \
	 fi
	python3 tools/serve-userguide.py --directory $(DEVGUIDE_OUT) --port $(GUIDE_PORT) --bind 127.0.0.1
