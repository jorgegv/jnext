...
`10.x.x.x`, `172.16-31.x.x`) are deliberately *not* refused - reaching a
machine on your own network is the main thing people use this for, and
nextsync in particular connects to a PC on the LAN.
...
next hard reset (**F1**, or **Machine \> Reset**) and on the next
...
`info` level it reports only the connections opened, refused and
closed - the events **NETWORKING** describes as never silent. `debug`
adds every AT command received and every response emitted, and `trace`
adds the raw byte traffic in both directions. Turning it down to `off`
silences the connection reports too, which is a deliberate choice on the
user’s part and not something jnext undoes.
