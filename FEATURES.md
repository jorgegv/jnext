...
- **`AT+CIPDOMAIN` resolves a name without connecting to it**, answering `+CIPDOMAIN:<address>`. It obeys exactly the same two rules a connection does — the `--esp-allow` list and the built-in address policy — so it can never be used to read back an address the emulator would refuse to dial, and a blocked lookup is indistinguishable from a name that does not exist
