...
- esxDOS answers for a directly loaded NEX, automatically (and for any other program with `--esxdos-stub`): the default drive, `run NAME.nex` chain-loading of another NEX in the same directory (game selectors / multi-part NEX games without booting NextZXOS), in-memory esxDOS file I/O for a game's config/score file, and an error (carry set) for every other esxDOS call instead of a crash into the 48K ROM
