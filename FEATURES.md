...
- **`AT+PING` really pings**, in process on an unprivileged ICMP socket rather than by running the system's `ping` — so it works in the Flatpak too, where the runtime ships no `ping` at all. It obeys the same rules everything else does: the `--esp-allow` list, the address policy, and needing a station. That last pair matters more than it sounds: without them a program could not *connect* to your router's admin page or a cloud metadata address, but could still confirm they exist and measure how far away they are. A host that is blocked and a host that is down answer identically, and a machine that will not permit the socket at all answers the same way rather than failing oddly
- **SNTP works against a real time server** (`AT+CIPSNTPCFG`, `AT+CIPSNTPTIME?`): a program can configure a server and timezone and ask what time it is, and jnext asks a real NTP server over UDP. **Note that this deliberately ignores `--rtc`** — that flag pins the emulated clock so screenshots stay reproducible, while SNTP reports the true wall-clock time a time server gives. The same `--esp-allow` list and address policy govern which server may be queried, and a refused or unreachable server reports the epoch, exactly as a module that has never synchronised does
...
- SNA: 48K and 128K snapshots with full register and paging restore. Saving follows the MACHINE: a 48K writes the 48K form (49179 bytes), a 128K and a +3 write the **128K form** — all eight banks, the 0xC000 paging register and PC in the extended header, so nothing is dropped — and a Next is refused outright, pointing at `.jns`. A state the format cannot describe is refused too, naming a format that can: a +3 in special paging or with ROM 2/3 paged (no 0x1FFD byte in an SNA) is sent to `.szx`, and a machine using EXTENDED PAGING via port 0xDFFD — whose bank at 0xC000 0x7FFD cannot name — to `.jns`, the only format that carries it. Validated against libspectrum — the library FUSE itself reads snapshots with — which accepts every form jnext writes and agrees with jnext's own `.szx` of the same machine, bank for bank.
...
- RZX: playback in every frontend, and recording on a **48K, 128K or +3** (IN replay, zlib compressed; 48K embeds an SNA, 128K/+3 an SZX). Recording is REFUSED on a Next, naming the machines that can: no snapshot an RZX can carry describes a Next, and its input log stores values without the ports they came from, so a Next recording is either lossy or silently out of step. Playing one an older jnext wrote still works, with a warning
