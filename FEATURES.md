...
- SNA: 48K and 128K snapshots with full register and paging restore. Saving follows the MACHINE: a 48K writes the 48K form (49179 bytes), a 128K and a +3 write the **128K form** — all eight banks, the 0xC000 paging register and PC in the extended header, so nothing is dropped — and a Next is refused outright, pointing at `.jns`. A +3 whose state the format cannot describe (special paging, or ROM 2/3 paged, neither of which has a byte in an SNA) is refused too, pointing at `.szx`. Validated against libspectrum, the library FUSE reads snapshots with
